/*
  ═══════════════════════════════════════════════════════════════════════════
  🔧 CORRECTIONS 3 PROBLÈMES IDENTIFIÉS
  ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF faq_count = 0 THEN
    ...
    RAISE NOTICE '   → Exécutez le script pour insérer des FAQ de test (voir ci-dessous)';
  ELSE
...
-- ═══════════════════════════════════════════════════════════════════════════
-- BONUS : Insérer 8 FAQ de test (si table vide)
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '⏰ PROBLÈME 3 : Suppression des CRON jobs qui échouent';
END $$;

-- Supprimer les 5 CRON jobs en échec (ils n'ont pas d'edge function associée)
DO $$
DECLARE
  deleted_count int := 0;
  job_name text;
  failed_jobs text[] := ARRAY[
...
BEGIN
  FOREACH job_name IN ARRAY failed_jobs
  LOOP
    DELETE FROM cron.job WHERE jobname = job_name;
    IF FOUND THEN
      deleted_count := deleted_count + 1;
      RAISE NOTICE '   ✓ Supprimé : %', job_name;
    END IF;
  END LOOP;

  RAISE NOTICE '';
  RAISE NOTICE '✅ % CRON jobs en échec supprimés', deleted_count;
  RAISE NOTICE '   → Ces jobs appelaient des Edge Functions qui n''existent pas';
  RAISE NOTICE '   → Système désormais plus propre et performant';
END $$;
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '🤖 BONUS : Création CRON job génération FAQ automatique';
END $$;

-- Supprimer l'ancien job s'il existe
SELECT cron.unschedule('ai-auto-generate-faq') WHERE EXISTS (
  SELECT 1 FROM cron.job WHERE jobname = 'ai-auto-generate-faq'
);
...
BEGIN
  ...
  SELECT COUNT(*) INTO cron_failed FROM cron.job WHERE active = false;
  ...
  RAISE NOTICE '   • CRON jobs en échec : %', cron_failed;
  RAISE NOTICE '';
  RAISE NOTICE '🎯 PROCHAINES ÉTAPES :';
  RAISE NOTICE '';
  RAISE NOTICE '1. IMAGES PEXELS';
  ...
  RAISE NOTICE '';
  RAISE NOTICE '2. VÉRIFIER PAGE FAQ';
  ...
  RAISE NOTICE '';
  RAISE NOTICE '3. VÉRIFIER CRON JOBS';
  RAISE NOTICE '   → Aller dans Supabase Dashboard → Database → Cron Jobs';
  RAISE NOTICE '   → Tous doivent être "Succeeded" (sauf si pas encore exécutés)';
  RAISE NOTICE '';
  RAISE NOTICE '4. TESTER GÉNÉRATION ARTICLE AVEC IMAGE';
  RAISE NOTICE '   → Après configuration PEXELS_API_KEY';
  RAISE NOTICE '   → Backoffice → Générateur IA → Mode Unifié';
  RAISE NOTICE '   → Mot-clé : "assurance taxi Paris"';
  RAISE NOTICE '   → L''article généré devrait avoir une image';
  RAISE NOTICE '';
  RAISE NOTICE '═══════════════════════════════════════════════════════════';
END $$;
...
-- Afficher les CRON jobs actifs
SELECT
  '⏰ CRON JOBS ACTIFS' as info,
  jobname,
  schedule,
  active
FROM cron.job
