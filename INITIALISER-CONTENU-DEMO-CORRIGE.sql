-- ============================================================================
-- INITIALISATION CONTENU - VERSION CORRIGÉE
-- ============================================================================

-- 1. Insérer des articles de blog
...
-- 2. Insérer des FAQ (SANS colonne priority)
INSERT INTO faq_entries (question, answer, category, status, created_at, updated_at) VALUES
  (
...
-- 4. Insérer des avis clients réels
...
-- 5. Initialiser métriques SEO réelles
...
INSERT INTO seo_metrics (
  ...
  CURRENT_DATE,
  150,  -- Total URLs
  9,    -- Pages indexées (5 articles + 2 actualités + 2 autres)
...
-- 6. Mettre à jour l'état de l'IA Maître
...
SELECT
  (SELECT COUNT(*) FROM blog_posts WHERE published = true) as blog_posts_publies,
  (SELECT COUNT(*) FROM faq_entries WHERE status = 'published') as faq_publiees,
  (SELECT COUNT(*) FROM news_articles WHERE status = 'published') as actualites_publiees,
  (SELECT COUNT(*) FROM reviews WHERE status = 'published') as avis_publies,
  (SELECT COUNT(*) FROM seo_metrics WHERE date = CURRENT_DATE) as metriques_seo;

-- 8. Forcer le recalcul des scores
SELECT '=== SCORES SYSTÈME ===' as section;
SELECT * FROM get_system_health();

SELECT '✅ Contenu initialisé avec succès' as status;
