...
function generateNewsletterHTML(articles: Article[], subscriberName: string): string {
...
<body style="margin: 0; padding: 0; font-family: Arial, sans-serif; background-color: #f4f4f4;">
  <div style="max-width: 600px; margin: 0 auto; background: white; padding: 40px;">
    ...
    <!-- Greeting -->
    <p style="color: #333; font-size: 16px; margin: 0 0 20px 0;">Bonjour ${subscriberName || 'cher lecteur'},</p>
  ...
  try {
    ...
    for (const send of sends) {
      try {
        ...
        let htmlContent = generateNewsletterHTML(articles, subscriber.first_name || '');
        
        // Ajouter le lien de désabonnement
        const unsubscribeLink = `https://taxiassur.com/newsletter/unsubscribe?token=${subscriber.unsubscribe_token}`;
        htmlContent = htmlContent.replace('{{unsubscribe_link}}', unsubscribeLink);
    ...
    console.log(`✅ Campagne ${campaign_id}: ${sentCount} envoyés, ${failedCount} échecs`);
