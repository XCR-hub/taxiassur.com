...
function extractNameFromEmail(
  ...
  if (parts.length >= 2) {
    return {
      firstName: parts[0].charAt(0).toUpperCase() +
        parts[0].slice(1).toLowerCase(),
      lastName: parts.slice(1).join(" ").charAt(0).toUpperCase() +
        parts.slice(1).join(" ").slice(1).toLowerCase(),
    };
  }
    ...
    for (const email of emails || []) {
      try {
        ...
        if (isLocallyBlacklisted(email.from_email)) {
          skipped++;
          await markEmailProcessed(supabase, email.id, email.metadata, {
            action: "blacklisted",
          });
          continue;
        }
        ...
        if (classifError) {
          console.error(
            `[auto-create-leads] Erreur classification ${email.from_email}:`,
            classifError,
          );
          skipped++;
          continue;
        ...
        console.log(
          `[auto-create-leads] ${email.from_email} → score=${score} confidence=${confidence} action=${action}`,
        );
        ...
        if (isRealLead && action === "create_lead") {
          ...
          if (upsertError) {
            console.error(
              `[auto-create-leads] Erreur upsert ${email.from_email}:`,
              upsertError,
            );
            continue;
          }
          ...
          if (isNew) {
            ...
            const { error: notificationError } = await supabase.from(
              ...
              message:
                `${firstName} ${lastName} a envoyé un email: "${email.subject}"`,
              priority: 1,
              read: false,
            });
            if (notificationError) {
              console.error(
                "New lead notification failed",
                notificationError.code || "unknown",
              );
            }
          } else {
            linked++;
          ...
          if (existingLead) {
            linked++;
            console.log(
              `[auto-create-leads] MEDIUM: lié au lead existant ${existingLead.id}`,
            );
            await linkEmailToLead(
              supabase,
              email,
              existingLead.id,
              confidence,
            ...
            skipped++;
            console.log(
              `[auto-create-leads] MEDIUM ignoré (pas de lead existant): ${email.from_email}`,
            );
            await markEmailProcessed(supabase, email.id, email.metadata, {
              action: "medium_no_existing",
              score,
              confidence,
              reasons,
            });
          ...
          rejected++;
          console.log(
            `[auto-create-leads] IGNORÉ (${confidence}, score=${score}): ${email.from_email}`,
          );
          await markEmailProcessed(supabase, email.id, email.metadata, {
            action: "rejected",
            score,
        ...
        console.error(
          `[auto-create-leads] Erreur ${email.from_email}:`,
          emailError?.message,
        );
      }
    }
...
async function linkEmailToLead(
  ...
  const { error: emailUpdateError } = await supabase.from("email_messages")
    .update({ lead_id: leadId })
    .eq("id", email.id);
  if (emailUpdateError) {
    throw new Error(
      "Email-to-lead update failed: " + (emailUpdateError.code || "unknown"),
    );
  }

  const { error: interactionError } = await supabase.from("crm_interactions")
    .insert({
      lead_id: leadId,
      type: "email",
      direction: "inbound",
  ...
  if (interactionError) {
    throw new Error(
      "Inbound interaction insert failed: " +
...
async function markEmailProcessed(
  ...
  if (error) {
    throw new Error(
      "Email classification persistence failed: " + (error.code || "unknown"),
    );
  }
}
