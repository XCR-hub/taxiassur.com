...
class IMAPClient {
  ...
  async fetchHeaders(start: number, end: number): Promise<Array<{ uid: string; from: string; subject: string; date: string; seq: number }>> {
    const response = await this.sendCommand(`FETCH ${start}:${end} (UID BODY.PEEK[HEADER.FIELDS (FROM SUBJECT DATE MESSAGE-ID)])`);
    const emails: Array<{ uid: string; from: string; subject: string; date: string; seq: number }> = [];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('🔄 SYNCHRONISATION COMPLÈTE DE TOUS LES EMAILS HISTORIQUES...');
    ...
    if (!account?.imap_password_encrypted) {
      return new Response(JSON.stringify({ success: false, error: 'Compte email non configuré', stats }), {
        status: 400,
    ...
    if (!await imap.connect(account.imap_host || 'imap.ionos.fr', account.imap_port || 993)) {
      return new Response(JSON.stringify({ success: false, error: 'Connexion IMAP échouée' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (!await imap.login(account.imap_username || 'team@taxiassur.com', account.imap_password_encrypted)) {
      await imap.close();
      return new Response(JSON.stringify({ success: false, error: 'Auth IMAP échouée' }), {
        status: 401,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (mailbox.exists > 0) {
      ...
      console.log(`📦 Traitement en ${totalBatches} batchs de ${BATCH_SIZE} emails`);

      for (let batchNum = 0; batchNum < totalBatches; batchNum++) {
        const start = batchNum * BATCH_SIZE + 1;
        const end = Math.min((batchNum + 1) * BATCH_SIZE, mailbox.exists);

        console.log(`📦 Batch ${batchNum + 1}/${totalBatches}: emails ${start} à ${end}`);
    ...
    const { data: linkResult } = await supabase.rpc('link_unassigned_emails_to_leads');
    if (linkResult) {
    ...
    return new Response(JSON.stringify({
      success: true,
      message: `✅ Sync historique: ${stats.inserted} nouveaux emails / ${stats.mailbox_total} total, ${stats.linked} liés`,
      stats,
