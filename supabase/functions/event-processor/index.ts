...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { event_type, lead_id, client_id, payload, source } = await req
      .json();

    console.log(`Event received: ${event_type} for lead ${lead_id}`);

    const { data: event } = await supabase.from("crm_events").insert({
      event_type,
    ...
    const { data: workflows } = await supabase.from("workflow_automations")
      .select("*").eq("is_active", true).eq("trigger_event", event_type);
    let autoActionsTriggered = 0;

    if (workflows && workflows.length > 0) {
      for (const workflow of workflows) {
        const shouldTrigger = await evaluateWorkflowConditions(
          supabase,
        ...
        if (shouldTrigger) {
          ...
          await supabase.from("workflow_automations").update({
    ...
    if (shouldInvolveCouncil(event_type)) {
      ...
      const councilResponse = await supabase.functions.invoke("ia-council", {
        body: { event_id: event.id, lead_id, event_type, context: payload },
      });
      if (councilResponse.data) {
        councilDecision = councilResponse.data.council_decision;
      }
    }
    ...
    return new Response(
      JSON.stringify({
        success: true,
        event_id: event.id,
        auto_actions_triggered: autoActionsTriggered,
        council_invoked: !!councilDecision,
...
async function evaluateWorkflowConditions(
  ...
  const { data: lead } = await supabase.from("leads").select("*").eq(
    "id",
    leadId,
  ).single();
  ...
  for (const [key, value] of Object.entries(conditions)) {
    ...
    if (key === "no_contact" && value === true) {
      const { data: interactions } = await supabase.from("crm_interactions")
    ...
    if (key === "documents_complete" && value === false) {
      const { data: docs } = await supabase.from("lead_documents").select(
        "document_type",
      ).eq("lead_id", leadId);
      const required = [
        "carte_grise",
      ...
      const hasAll = required.every((type) =>
        docs?.some((d: { document_type: string | null }) =>
...
async function executeHighPriorityActions(supabase: any, eventId: string) {
  const { data: actions } = await supabase.from("crm_actions").select("*").eq(
    "event_id",
    eventId,
  ...
  for (const action of actions) {
    try {
      ...
      await supabase.from("crm_actions").update({
...
async function executeAction(supabase: any, action: any) {
  const actionMap: Record<string, string> = {
    ...
    "send_sms_reminder": "send_sms",
    "make_phone_call": "call",
    "send_quote": "send_quote",
    "request_missing_documents": "request_documents",
    "send_loyalty_offer": "send_loyalty_offer",
    "ai_voice_call": "voice_call",
  };
  const mappedAction = actionMap[action.action_type] || action.action_type;
  try {
    const response = await supabase.functions.invoke("ia-auto-executor", {
      body: {
        action: mappedAction,
