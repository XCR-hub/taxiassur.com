...
async function collectAllMetrics(supabase: any) {
  ...
  const successfulAutomations =
    automationHistory?.filter((a: any) => a.status === "success").length || 0;
  ...
  metrics.lead_metrics = {
    ...
    conversion_rate: totalLeads > 0
      ? ((convertedLeads || 0) / totalLeads * 100).toFixed(2)
      : 0,
  };
  ...
  metrics.automation_metrics = {
    ...
    success_rate: totalAutomations > 0
      ? ((successfulAutomations / totalAutomations) * 100).toFixed(2)
      : 0,
  };

  const { data: topRules } = await supabase.rpc("get_top_performing_rules", {
    limit_param: 5,
  });
...
async function storeMetrics(supabase: any, metrics: any) {
  const metricsToStore = [
    ...
    },
    {
      ...
      threshold_min: 5,
      threshold_max: 100,
      status: parseFloat(metrics.lead_metrics.conversion_rate) >= 5
        ? "healthy"
        : "warning",
    },
    {
      ...
      threshold_min: 70,
      threshold_max: 100,
      status: parseFloat(metrics.automation_metrics.success_rate) >= 70
        ? "healthy"
        : "critical",
    },
    {
  ...
  for (const anomaly of anomalies) {
    ...
    if (!autoHandled && anomaly.severity === "critical") {
      await supabase.from("smart_alerts").insert({
        ...
        description:
          `La métrique ${anomaly.metric_name} est à ${anomaly.actual_value} (attendu: ${anomaly.expected_value})`,
...
async function attemptAutoResolution(supabase: any, anomaly: any) {
  console.log(
    `[Monitoring] Attempting auto-resolution for: ${anomaly.anomaly_type}`,
  );
...
async function makePredictions(supabase: any, metrics: any) {
  ...
  const currentConversionRate = parseFloat(
    metrics.lead_metrics.conversion_rate,
  );
  ...
  predictions.push({
    ...
    predicted_value: currentConversionRate *
      (trend === "increasing" ? 1.1 : 0.9),
...
async function updateBaselines(supabase: any, metrics: any) {
  const baselines = [
    ...
    },
    {
      ...
      current_value: parseFloat(metrics.lead_metrics.conversion_rate),
      baseline_value: 7,
      trend_direction: parseFloat(metrics.lead_metrics.conversion_rate) > 7
        ? "up"
        : "down",
    },
  ];
  ...
  for (const baseline of baselines) {
    const improvement = ((baseline.current_value - baseline.baseline_value) /
      baseline.baseline_value * 100).toFixed(2);
