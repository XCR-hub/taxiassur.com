...
const OPENAI_GPT4O: ProviderConfig = {
  id: "openai",
  model: "gpt-4o",
  label: "GPT-4o",
};
const OPENAI_GPT4O_MINI: ProviderConfig = {
  id: "openai",
  model: "gpt-4o-mini",
  label: "GPT-4o Mini",
};
const ANTHROPIC_SONNET: ProviderConfig = {
  id: "anthropic",
  model: "claude-sonnet-4-20250514",
  label: "Claude Sonnet",
};
const ANTHROPIC_HAIKU: ProviderConfig = {
  id: "anthropic",
  model: "claude-3-5-haiku-20241022",
  label: "Claude Haiku",
};
const GEMINI_PRO: ProviderConfig = {
  id: "gemini",
  model: "gemini-2.0-flash",
  label: "Gemini 2.0 Flash",
};
const GEMINI_FLASH: ProviderConfig = {
  id: "gemini",
  model: "gemini-1.5-flash",
  label: "Gemini 1.5 Flash",
};
const HF_MISTRAL: ProviderConfig = {
  id: "huggingface",
...
const SYSTEM_PROMPT =
  `Tu es un moteur de decision IA multi-agents pour TaxiAssur, courtier en assurance taxi en France.
...
async function callOpenAI(prompt: string, model: string): Promise<string> {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `OpenAI ${model} error ${res.status}: ${errBody.slice(0, 200)}`,
    );
  }
...
async function callAnthropic(prompt: string, model: string): Promise<string> {
  ...
  if (!res.ok) {
    const errBody = await res.text().catch(() => "");
    throw new Error(
      `Anthropic ${model} error ${res.status}: ${errBody.slice(0, 200)}`,
    );
  }
  const data = await res.json();
...
async function callGemini(prompt: string, model: string): Promise<string> {
  const url =
    `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${PROVIDER_KEYS.gemini}`;
  ...
  if (!res.ok) {
    const errBody = await res.text().catch(() => "");
    throw new Error(
      `Gemini ${model} error ${res.status}: ${errBody.slice(0, 200)}`,
    );
  }
  const data = await res.json();
...
async function callHuggingFace(prompt: string, model: string): Promise<string> {
  ...
  if (!res.ok) {
    const errBody = await res.text().catch(() => "");
    throw new Error(
      `HuggingFace ${model} error ${res.status}: ${errBody.slice(0, 200)}`,
    );
  }
  const data = await res.json();
...
async function callWithFallback(
  ...
  for (const provider of providers) {
    try {
      ...
      console.warn(
        `[${agent.id}] ${provider.label} failed: ${err}. Trying next provider...`,
      );
    }
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const recentMap = new Set<string>(
      (existingDecisions ?? []).map((d: { lead_id: string; agent: string }) =>
        `${d.lead_id}::${d.agent}`
      ),
    );
    ...
    for (const lead of leads) {
      const pendingAgentIds = agentsToRun.filter((agentId) =>
        !recentMap.has(`${lead.id}::${agentId}`)
      );
...
Fichiers uploades: ${lead.total_uploaded_files ?? 0} (${
        lead.validated_files ?? 0
      } valides)
Documents complementaires en attente: ${
        lead.documents_complementaires_pending ?? 0
      }
...
`.trim();
      ...
      const agentResults = await Promise.allSettled(
        pendingAgents.map(async (agent) => {
          const singleAgentPrompt = prompt.replace(
            `Agents a executer:\n${agentDescriptions}`,
            `Agent a executer:\n- Agent "${agent.id}" (${agent.name}): ${agent.focus} -> type de decision: "${
              (DECISION_TYPES[agent.id] ?? ["suggestion"])[0]
            }"`,
          );
      ...
      for (const result of agentResults) {
        ...
        try {
          ...
          for (const d of decisions) {
            ...
            const validType = types.includes(d.decision_type)
              ? d.decision_type
              : types[0];

            const conf = Math.min(
              0.99,
              Math.max(0.5, parseFloat(d.confidence_score) || 0.8),
            );
            ...
            allDecisions.push({
              ...
              actions: actionText
                ? [{ type: validType, action: actionText }]
                : [],
              suggested_action: actionText,
              data_sources: Array.isArray(d.data_sources)
                ? d.data_sources
                : ["crm_leads"],
              status: "pending",
            ...
            providerStats[provider.label] =
              (providerStats[provider.label] ?? 0) + 1;
          }

          if (parsed.lead_assessment) {
            const la = parsed.lead_assessment;
            leadScores.push({
              ai_lead_score: typeof la.ai_lead_score === "number"
              ...
              ai_conversion_probability:
                typeof la.ai_conversion_probability === "number"
                  ? Math.min(
                    1,
                    Math.max(0, parseFloat(la.ai_conversion_probability)),
                  )
                  : undefined,
              ai_risk_level:
                ["LOW", "MEDIUM", "HIGH"].includes(la.ai_risk_level)
                  ? la.ai_risk_level
                  : undefined,
            });
          }
          ...
          console.error(
            `[${agent.id}] JSON parse error from ${provider.label}:`,
            parseErr,
          );
        }
      }
      ...
      if (leadScores.length > 0) {
        ...
        const probs = leadScores.filter((s) =>
          s.ai_conversion_probability !== undefined
        );
        const risks = leadScores.filter((s) => s.ai_risk_level !== undefined);

        const avgScore = scores.length > 0
          ? Math.round(
            scores.reduce((s, v) => s + (v.ai_lead_score ?? 0), 0) /
              scores.length,
          )
          : undefined;
        ...
        const avgProb = probs.length > 0
          ? parseFloat(
            (probs.reduce((s, v) => s + (v.ai_conversion_probability ?? 0), 0) /
              probs.length).toFixed(2),
          )
          : undefined;
        ...
        risks.forEach((r) => {
          if (r.ai_risk_level) {
            riskVotes[r.ai_risk_level] = (riskVotes[r.ai_risk_level] ?? 0) + 1;
          }
        });
        const consensusRisk = Object.entries(riskVotes).sort((a, b) =>
          b[1] - a[1]
        )[0]?.[0];
    ...
    if (allDecisions.length > 0) {
      const { error: insertErr } = await supabase.from("crm_ai_decisions")
        .insert(allDecisions);
    ...
    return new Response(
      JSON.stringify({
        ...
        message:
          `${inserted} decisions generees pour ${leads.length} leads via ${
