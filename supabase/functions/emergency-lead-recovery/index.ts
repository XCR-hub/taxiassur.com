...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: emergencyActions } = await supabase.rpc('trigger_emergency_actions');
    console.log("✅ Actions urgentes déclenchées:", emergencyActions);

    const { data: blockers } = await supabase.rpc('analyze_conversion_blockers');
      ...
      await supabase.from('ai_optimization_queue').insert([
        {
          ...
          expected_impact: { conversion_increase: '+500%', urgency: 'CRITICAL' }
        },
        {
    ...
    const severity = droughtData.hours_since_last_lead > 48 ? 'CRITICAL' : 
                     droughtData.hours_since_last_lead > 24 ? 'HIGH' : 'MEDIUM';
