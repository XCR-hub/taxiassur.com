...
const STYLE_INSTRUCTIONS: Record<string, string> = {
  professionnel: `Ton formel mais pas robotique. Vocabulaire expert. Phrases complexes avec subordonnees.
Comme un courtier senior qui ecrit dans une revue pro. Tu ADORES les details techniques.`,
  accessible: `Ton chaleureux et direct. Vocabulaire simple. Tu parles comme un ami expert.
"Bon, soyons concrets", "Franchement", "En gros". Tu vulgarises sans simplifier.`,
  expert: `Ton autoritaire et precis. Tu assumes tes opinions. Vocabulaire technique.
"Il faut le dire clairement", "Les chiffres parlent d'eux-memes". Tu cites des sources.`,
  conversationnel: `Ton decontracte. Phrases courtes. Parfois une phrase-choc de 3 mots. Puis une longue explication.
"Ecoutez", "Pas de blabla", "Concretement". Tu interpelles le lecteur.`,
  pedagogique: `Ton patient et clair. Tu expliques etape par etape. Analogies et exemples.
...
function buildAntiAISystemPrompt(
  ...
  const instruction = STYLE_INSTRUCTIONS[style.name] || STYLE_INSTRUCTIONS.professionnel;
...
function calculateNaturalnessScore(content: string): number {
  ...
  const humanMarkers = [
    { pattern: /franchement|bon,|ecoutez|pas de secret|en gros|concretement/i, points: 8 },
    { pattern: /selon moi|a mon avis|je pense que|personnellement/i, points: 10 },
  ...
  const paragraphs = content.split(/<\/p>|<\/h[23]>/).filter(p => p.trim().length > 20);
  if (paragraphs.length > 3) {
    ...
    if (stdDev > avgLen * 0.4) score += 10;
  }

  const sentences = content.replace(/<[^>]+>/g, '').split(/[.!?]+/).filter(s => s.trim().length > 5);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!masterPrompt) {
      masterPrompt = `Tu es un VRAI expert humain francais avec 15+ ans d'experience en assurance taxi.
...
Ajoute des anecdotes credibles. Pose des questions au lecteur.`;
      ...
      try {
        ...
        if (pexelsResponse.ok) {
          ...
          if (pexelsData.photos?.length > 0) {
            ...
            imageAlt = photo.alt || `${keyword} ${city} - Photo professionnelle`;
          }
        }
      } catch (error) {
    ...
    const introHook = INTRO_HOOKS[Math.floor(Math.random() * INTRO_HOOKS.length)];
    const structurePattern = STRUCTURE_PATTERNS[Math.floor(Math.random() * STRUCTURE_PATTERNS.length)];
    ...
    const userPrompt = buildUserPrompt(
      keyword, city, secondaryKeywords,
      dept, deptName, region, population, taxiCount,
    ...
    if (!openaiApiKey) {
      const fallbackContent = {
        blogPost: {
          ...
          content: `<h2>Introduction</h2><p>Guide sur ${keyword} a ${city}.</p>`,
