  ...
  try {
    ...
    if (performance.trend === "critical") {
      ...
      decisions.push({
        ...
        action_taken:
          "Activation mode conversion ultra-agressif + génération contenu urgente",
    ...
    if (popupData && popupData.length > 0) {
      ...
      const converted = popupData.filter((p) =>
        p.action === "converted"
      ).length;
    ...
    if (analyticsData && analyticsData.length > 0) {
      const avgDuration = analyticsData.reduce((sum, p) =>
        sum + (p.duration_seconds || 0), 0) / analyticsData.length;
    ...
    const topKeywords = [
      ...
      {
        keyword: "comparateur assurance taxi",
        volume: 600,
        difficulty: 55,
        position: 18,
      },
    ...
    for (const kw of topKeywords) {
      const priorityScore = Math.min(
        100,
        Math.floor(
          (kw.volume / 50) + (100 - kw.difficulty) +
            (kw.position > 10 ? 30 : 10),
        ),
      );
      ...
      await supabase.from("ai_keywords_strategy").upsert({
        ...
        ai_strategy:
          `Créer 3 articles optimisés + backlinks autorité + meta descriptions killer`,
        status: "active",
      }, { onConflict: "keyword" });
