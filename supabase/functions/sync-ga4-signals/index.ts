  ...
  try {
    ...
    if (!serviceAccountEmail || !serviceAccountKey) {
      return new Response(JSON.stringify({
        success: false,
        message: "GOOGLE_SERVICE_ACCOUNT_EMAIL and GOOGLE_SERVICE_ACCOUNT_KEY are required. Make sure the service account has 'Viewer' access in GA4 Admin.",
        setup_required: true,
    ...
    if (!ga4PropertyId) {
      return new Response(JSON.stringify({
        success: false,
        message: "GA4_PROPERTY_ID secret is required. Find it in GA4 Admin > Property Settings.",
        setup_required: true,
      }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
