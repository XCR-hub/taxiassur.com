...
function calculateRelevanceScore(title: string, content: string, keywords: string[]): number {
  ...
  const highPriorityKeywords = ['assurance taxi', 'réglementation', 'décret', 'tarif taxi'];
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const source of (sources as NewsSource[])) {
      try {
        const now = new Date();
        const lastCheck = source.last_check ? new Date(source.last_check) : new Date(0);
        const timeSinceLastCheck = (now.getTime() - lastCheck.getTime()) / 1000;

        if (timeSinceLastCheck < source.check_interval) {
          console.log(`Skipping ${source.name} - checked ${Math.floor(timeSinceLastCheck)}s ago`);
          continue;
        }
        ...
        if (source.type === 'rss') {
          const rssResponse = await fetch(`${supabaseUrl}/functions/v1/rss-parser`, {
            method: 'POST',
            headers: {
          ...
          if (rssResponse.ok) {
            ...
            if (rssData.success && rssData.items) {
              for (const item of rssData.items) {
                const score = calculateRelevanceScore(item.title, item.description, source.keywords);
          ...
          const linkedinResponse = await fetch(`${supabaseUrl}/functions/v1/linkedin-scraper`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${supabaseServiceKey}`,
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `Agrégation terminée : ${newArticles.length} nouveaux articles sur ${totalProcessed} traités`,
        stats: {
