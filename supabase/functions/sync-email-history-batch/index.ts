...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (mailbox.exists > 0) {
      ...
      console.log(
        `Fetching emails from ${start} to ${end} (total: ${mailbox.exists}, offset: ${offset})`,
      );
    ...
    const progress = Math.round(
      ((stats.total_in_mailbox -
        (stats.total_in_mailbox - offset - stats.batch_retrieved)) /
        stats.total_in_mailbox) * 100,
    );
