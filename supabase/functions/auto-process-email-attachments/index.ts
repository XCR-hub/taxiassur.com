...
const DOC_PATTERNS: DocumentTypeMapping[] = [
  {
    pattern: /permis.*conduire|driving.*license|permis/i,
    type: "permis_conduire",
    confidence: 0.85,
  },
  {
    pattern:
      /carte.*grise|registration|immatriculation|certificat.*temporaire/i,
    type: "carte_grise",
    confidence: 0.9,
  },
  { pattern: /rib|iban|bank.*details?/i, type: "rib", confidence: 0.9 },
  {
    pattern: /releve.*info|insurance.*history/i,
    type: "releve_information",
    confidence: 0.8,
  },
  { pattern: /kbis|sirene|siret/i, type: "kbis", confidence: 0.95 },
  {
    pattern: /carte.*pro|professional.*card|licence.*taxi/i,
    type: "carte_professionnelle",
    confidence: 0.9,
  },
  {
    pattern: /identite|cni|id.*card|passport|passeport/i,
    type: "piece_identite",
    confidence: 0.85,
  },
  {
    pattern: /justif.*domicile|proof.*address|facture/i,
    type: "justificatif_domicile",
    confidence: 0.8,
  },
  {
    pattern: /autorisation.*stationnement|parking/i,
    type: "autorisation_stationnement",
    confidence: 0.85,
  },
];
...
function classifyDocument(
  ...
  const normalized = filename.toLowerCase().normalize("NFD").replace(
    /[\u0300-\u036f]/g,
    "",
  );
...
function shouldFilterAttachment(
  filename: string,
  mimeType: string,
  size: number,
): boolean {
  ...
  if (
    mt.includes("pdf") || mt.includes("word") || mt.includes("document") ||
    mt.includes("spreadsheet") || mt.includes("officedocument")
  ) return false;
  ...
  const docImg = [
    ...
    /permis|carte|rib|kbis|identite|justif|certificat/i,
  ];
  for (const p of docImg) if (p.test(fn)) return false;

  if (
    mt.startsWith("image/") && !fn.match(/\d{4,}/) && size > 0 && size < 500000
  ) return true;
  return false;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(
      "Traitement automatique des pièces jointes (lecture email_attachments)...",
    );
    ...
    const { data: attachments, error: fetchError } = await supabase
      ...
      .select(
        "id, email_message_id, filename, content_type, file_size, storage_path, proposed_doc_type, classification_confidence, status, assigned_document_id",
      )
    ...
    for (const att of attachments || []) {
      ...
      try {
        ...
        if (
          shouldFilterAttachment(att.filename, att.content_type, att.file_size)
        ) {
        ...
        const classification = att.proposed_doc_type
          ? {
            type: att.proposed_doc_type,
            confidence: Number(att.classification_confidence) || 0.7,
          }
        ...
        if (existingDoc) {
          await supabase.from("email_attachments")
            .update({
              assigned_document_id: existingDoc.id,
              status: "assigned",
            })
            .eq("id", att.id);
          skipped++;
