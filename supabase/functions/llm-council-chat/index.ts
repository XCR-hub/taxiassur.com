  ...
  switch (provider) {
    ...
    case "mistral":
      return callHuggingFace(
        modelId.replace(/^(huggingface|meta|mistral)\//, ""),
        messages,
        config,
      );
    default:
...
function getChairmanPrompt(
  ...
  const responsesText = responses
    .filter((r) => r.content && !r.error)
    .map((r) => {
      const avgScores = rankings.flatMap((rk) => rk.rankings || []).filter(
        (rk) => rk.anonymous_id === r.anonymous_id,
      );
      const avgAcc = avgScores.length > 0
        ? avgScores.reduce((a, b) => a + (b.accuracy_score || 0), 0) /
          avgScores.length
        : 0;
      return `\n=== ${r.display_name} (Accuracy: ${
        avgAcc.toFixed(1)
      }) ===\n${r.content}\n`;
    })
    .join("\n");
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const chairmanConfig = configs.find((c) => c.is_chairman) ||
      councilModels[0];
    ...
    console.log(
      `[LLM Council] Stage 1: Gathering ${councilModels.length} responses...`,
    );
    ...
    const messages = [
      {
        role: "system",
        content:
          "You are an expert AI assistant for TaxiAssur, a French taxi insurance company. Provide accurate, insightful, and helpful responses in French when the query is in French.",
      },
      { role: "user", content: query },
    ];

    const responsePromises = councilModels.slice(0, 6).map((config) =>
    ...
    console.log(
      `[LLM Council] Stage 1 complete: ${validResponses.length}/${responses.length} successful`,
    );
    ...
    if (!single_turn && validResponses.length > 1) {
      ...
      const reviewMessages = [
        {
          role: "system",
          content: "You are an expert judge. Respond only with valid JSON.",
        },
      ...
      const chairmanMessages = [
        {
          role: "system",
          content:
            "You are the Chairman of an LLM Council. Synthesize multiple AI responses into one comprehensive answer.",
        },
      ...
      if (allScores.length > 0) {
        const avgScore = allScores.reduce((a, b) =>
          a +
          ((b.accuracy_score || 0) + (b.insight_score || 0) +
              (b.clarity_score || 0)) / 3, 0) / allScores.length;
        consensusScore = avgScore * 10;
      }

      console.log(
        `[LLM Council] Stage 3 complete. Consensus: ${
          consensusScore.toFixed(1)
        }%`,
      );
    } else {
      const bestResponse = validResponses.sort((a, b) =>
        b.tokens_used - a.tokens_used
      )[0];
      finalResponse = bestResponse?.content ||
        "No valid responses received from the council.";
