/*
  # Réactivation du Système d'Emails Brevo (CORRIGÉ)

  1. Correctif
    - Correction du bug `to_char()` avec `format()`
    - Utilisation de concaténation au lieu de format() pour la date
    - Réactivation du trigger send_lead_notification_emails

  2. Fonctionnalités
    - Email de confirmation au client via Brevo
    - Email de notification à l'équipe via Brevo
    - Gestion d'erreurs robuste (non-bloquante)

  3. Sécurité
    - Fonction SECURITY DEFINER pour accès pg_net
    - Pas de blocage si l'envoi échoue
    - Logging des erreurs
*/

-- Recréer la fonction en corrigeant le bug format()
...
BEGIN
  ...
  -- Si pas configuré, utiliser les valeurs par défaut
  IF v_supabase_url IS NULL THEN
    v_supabase_url := 'https://drohhxrkoequjphvabvq.supabase.co';
  END IF;

  -- CORRECTION: Formater la date AVANT de l'utiliser dans format()
  v_formatted_date := to_char(NEW.created_at, 'DD/MM/YYYY à HH24:MI');
...
<body>
  ...
  BEGIN
    ...
    RAISE NOTICE 'Email de confirmation Brevo envoyé à: %', NEW.email;
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'Erreur lors de l''envoi de l''email client via Brevo: %', SQLERRM;
  END;
  ...
  BEGIN
    ...
    RAISE NOTICE 'Email de notification équipe Brevo envoyé';
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'Erreur lors de l''envoi de l''email équipe via Brevo: %', SQLERRM;
  END;
...
-- Réactiver le trigger sur INSERT
...
'RÉACTIVÉE - Envoie automatiquement des emails via Brevo lors de la création d''un nouveau lead. Corrige le bug to_char() avec format().';

COMMENT ON TRIGGER trigger_send_lead_emails ON leads IS
'Déclenche l''envoi automatique d''emails Brevo lors de la création d''un nouveau lead';
