/*
  # Fix prospect email confirmation - Field name mismatch
  
  1. Problem
    - Edge function checks for payload.type or notification.type
    - But migration sends event_type instead of type
    - Result: notificationType defaults to 'upload' instead of 'confirmation'
  
  2. Solution
    - Change 'event_type' to 'type' in the JSON payload
  
  3. Impact
    - Prospect will now receive confirmation email after upload
*/
...
BEGIN
  ...
  LOOP
    ...
    -- 📧 EMAIL 2 : Confirmation pour le prospect (avec le bon nom de champ!)
...
'Envoie 2 emails : (1) notification admin à team@taxiassur.com, (2) confirmation prospect - FIXED: uses "type" instead of "event_type"';
