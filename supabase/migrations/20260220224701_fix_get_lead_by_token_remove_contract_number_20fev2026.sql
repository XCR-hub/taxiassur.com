/*
  # Fix get_lead_by_token - Retirer contract_number inexistante - 20 février 2026

  ## Problème identifié

  La colonne `contract_number` n'existe pas dans crm_leads.

  ## Solution

  Retirer contract_number du RETURNS TABLE et de la requête SELECT
*/

-- Supprimer l'ancienne fonction
DROP FUNCTION IF EXISTS public.get_lead_by_token(text);

-- Créer la fonction sans contract_number
...
BEGIN
  ...
  IF v_lead_id IS NOT NULL THEN
    ...
    -- Compter les TYPES de documents distincts VALIDÉS
    SELECT COUNT(DISTINCT d.document_type)
    INTO v_validated_docs
    FROM crm_lead_documents d
    WHERE d.lead_id = v_lead_id
      AND (d.validated_at IS NOT NULL OR d.status = 'validated')
      AND d.document_type IN (
        'licence_taxi',
        'permis_conduire',
        'piece_identite',
...
'Récupère un lead par son access_token avec compteurs de documents';
