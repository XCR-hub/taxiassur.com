/*
  # Fix get_lead_by_token - Ajout des compteurs de documents

  ## Problème
  L'espace prospect affiche "0/7" alors que des documents ont été uploadés.
  La fonction get_lead_by_token ne calcule pas les champs :
  - uploaded_documents (nombre de documents uploadés)
  - total_documents (nombre total de documents requis = 7)
  - progression_percentage (pourcentage de complétion)

  ## Solution
  Ajouter le calcul de ces champs en comptant les documents dans crm_lead_documents
*/

-- Supprimer l'ancienne fonction
DROP FUNCTION IF EXISTS public.get_lead_by_token(text);

-- Recréer avec les compteurs de documents
...
DECLARE
  v_lead_id uuid;
  v_total_docs integer := 7; -- 7 documents requis par défaut
...
BEGIN
  ...
  IF v_lead_id IS NOT NULL THEN
    -- Compter les documents validés (uploaded_at IS NOT NULL)
    SELECT COUNT(*)
...
'Récupère les informations complètes d''un lead via son access_token avec compteurs de documents';
