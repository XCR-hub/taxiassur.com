/*
  # Fix validate_quote_by_token et refuse_quote_by_token - Colonnes correctes
  
  1. Problème
    - Les fonctions utilisent les anciennes colonnes
    - status → quote_status
    - company_id → insurance_company_id
    - refused_at → quote_refused_at
...
-- ==================================================
-- FONCTION: Valider un devis via token
-- ==================================================
...
BEGIN
  ...
  INTO 
    ...
    v_quote_amount
  FROM lead_company_quotes lcq
  INNER JOIN insurance_companies ic ON ic.id = lcq.insurance_company_id
  INNER JOIN crm_leads cl ON cl.id = lcq.lead_id
  WHERE lcq.id = p_quote_id;
...
-- ==================================================
-- FONCTION: Refuser un devis via token
-- ==================================================
...
COMMENT ON FUNCTION validate_quote_by_token(uuid, text) IS 
'Valide un devis depuis l''espace prospect avec les colonnes correctes (insurance_company_id, quote_status)';

COMMENT ON FUNCTION refuse_quote_by_token(uuid, text, text) IS 
'Refuse un devis depuis l''espace prospect avec les colonnes correctes (insurance_company_id, quote_status)';