/*
  # Fix classify_attachment - Correct Status Value
  
  1. Problem
    - classify_attachment uses status 'pending_validation'
    - But crm_lead_documents constraint only allows: 'pending', 'validated', 'rejected'
    - This causes the drag & drop to fail
  
  2. Solution
    - Change 'pending_validation' to 'pending' in classify_attachment function
*/
  ...
  IF p_create_document THEN
    ...
    IF v_document_id IS NOT NULL THEN
      ...
      SET
        ...
        status = 'pending',  -- ✅ Changed from 'pending_validation' to 'pending'
        uploaded_at = now(),
        updated_at = now()
    ...
    ELSE
      ...
      VALUES (
        ...
        v_filename,
        v_storage_path,
        v_file_size::integer,  -- ✅ Cast to integer
        v_content_type,
        'pending',  -- ✅ Changed from 'pending_validation' to 'pending'
        now()
      )
...
'Classifie un document depuis email_attachments ou prospect_documents et le copie dans crm_lead_documents - FIXED status constraint';
