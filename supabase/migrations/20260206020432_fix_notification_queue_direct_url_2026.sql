/*
  # Correction de la fonction avec URL directe au lieu de current_setting

  Utilise l'URL Supabase directement au lieu de current_setting qui n'existe pas
*/
...
DECLARE
  ...
  v_supabase_url text := 'https://xpmvtmtlscqelxhwuqcc.supabase.co';
  v_processed int := 0;
  v_errors int := 0;
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Envoyer via net.http_post sans utiliser de service role key dans l'URL
      -- L'edge function sera appelée avec net capabilities
...
COMMENT ON FUNCTION process_document_notification_queue() IS 'Traite les notifications en attente et envoie les emails aux prospects (URL directe)';
