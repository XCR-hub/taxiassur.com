/*
  # Fix upsert_lead pour envoyer les emails manuellement
  
  1. Problème identifié
    - Le trigger ne se déclenche PAS lors des INSERT depuis upsert_lead (SECURITY DEFINER)
    - Le trigger fonctionne avec les INSERT directs
  
  2. Solution
    - Modifier upsert_lead pour appeler queue_simple_email manuellement
    - Envoyer 2 emails : équipe + prospect
    - Uniquement pour les NOUVEAUX leads (is_new = true)
  
  3. Résultat attendu
    - Chaque nouveau lead génère 2 emails automatiquement
    - Les emails sont dans la queue en statut "pending"
    - Le processeur de queue les envoie via IONOS
*/
...
BEGIN
  ...
  ELSE
    ...
    -- ============================================
    -- ENVOYER LES EMAILS MANUELLEMENT
    -- ============================================
    ...
    BEGIN
      ...
      v_team_queue_id := queue_simple_email(
        ...
        p_subject := format('🚨 NOUVEAU LEAD: %s - %s', v_full_name, p_city),
...
',
  ...
  RAISE LOG '[UPSERT_LEAD] 🎉 Fin - Lead ID: %, Is New: %, Emails: %', v_lead_id, v_is_new, CASE WHEN v_is_new THEN 'Envoyés' ELSE 'Non envoyés' END;
...
COMMENT ON FUNCTION upsert_lead IS 
'Crée ou met à jour un lead. Pour les nouveaux leads, envoie automatiquement 2 emails : équipe + prospect';

-- Nettoyage des emails de test
DELETE FROM email_queue WHERE to_email = 'test@example.com';
DELETE FROM email_queue WHERE to_email LIKE 'test-direct-insert-%';
DELETE FROM crm_leads WHERE email LIKE 'test-direct-insert-%';