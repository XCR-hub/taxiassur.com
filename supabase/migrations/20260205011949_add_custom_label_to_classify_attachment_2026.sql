/*
  # Add custom_label support to classify_attachment function

  ## Changes
  - Add p_custom_label parameter to classify_attachment function
  - Store custom_label when creating/updating documents in crm_lead_documents
  
  ## Purpose
  - Allow commercials to classify attachments as custom documents
  - Store the custom document label for display
*/
...
BEGIN
  ...
  IF p_create_document THEN
    -- Check if document already exists (for custom docs, also check custom_label)
    IF p_doc_type = 'custom' AND p_custom_label IS NOT NULL THEN
      SELECT id INTO v_document_id
      FROM crm_lead_documents
      WHERE lead_id = v_lead_id
        AND document_type = p_doc_type
        AND custom_label = p_custom_label
        AND status != 'rejected'
      LIMIT 1;
    ELSE
      SELECT id INTO v_document_id
    ...
    ELSE
      -- Create new document WITH bucket and custom_label
...
COMMENT ON FUNCTION classify_attachment(uuid, text, boolean, text) IS 
'Classifie une pièce jointe et crée un document dans crm_lead_documents avec support des documents personnalisés';
