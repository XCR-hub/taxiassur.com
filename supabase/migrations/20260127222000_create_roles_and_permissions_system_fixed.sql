...
INSERT INTO system_modules (slug, name, description, icon, display_order) VALUES
  ...
  ('crm_pipeline', 'Pipeline Commercial', 'Kanban et suivi', '🎯', 3),
  ('crm_inbox', 'Inbox Multicanal', 'Emails, SMS, WhatsApp', '📧', 4),
  ('crm_documents', 'Documents', 'Gestion documents', '📄', 5),
  ('crm_quotes', 'Devis & Contrats', 'Création devis', '💰', 6),
  ('crm_production', 'Production', 'Validation production', '🏭', 7),
  ('crm_retention', 'Rétention', 'Suivi renouvellements', '🔄', 8),
  ('crm_settings', 'Paramètres CRM', 'Configuration CRM', '⚙️', 9),
  ('analytics', 'Analytics', 'Statistiques', '📈', 10),
  ('seo', 'SEO', 'Référencement', '🔍', 11),
  ('content', 'Contenu', 'Gestion contenu', '✍️', 12),
  ('social_media', 'Réseaux Sociaux', 'Publication réseaux', '📱', 13),
  ('email_marketing', 'Email Marketing', 'Campagnes email', '📬', 14),
  ('marketplace', 'Marketplace', 'Place de marché', '🛍️', 15),
  ('ai_tools', 'Outils IA', 'Intelligence artificielle', '🤖', 16),
  ('admin_users', 'Utilisateurs', 'Gestion utilisateurs', '👤', 17),
  ('admin_roles', 'Rôles', 'Configuration accès', '🔐', 18),
  ('admin_integrations', 'Intégrations', 'APIs webhooks', '🔗', 19),
...
DROP POLICY IF EXISTS "Tous les utilisateurs peuvent voir les modules" ON system_modules;
DROP POLICY IF EXISTS "Seuls les masters peuvent modifier les modules" ON system_modules;
DROP POLICY IF EXISTS "Tous peuvent voir les permissions de rôle" ON role_permissions;
DROP POLICY IF EXISTS "Seuls les masters peuvent modifier les permissions de rôle" ON role_permissions;
DROP POLICY IF EXISTS "Les utilisateurs peuvent voir leurs permissions personnalisées" ON user_custom_permissions;
DROP POLICY IF EXISTS "Seuls les masters/managers peuvent modifier les permissions utilisateurs" ON user_custom_permissions;
