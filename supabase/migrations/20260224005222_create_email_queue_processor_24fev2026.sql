/*
  # Processeur de queue d'emails
  
  1. Fonction
    - process_email_queue_simple(): Traite les emails en attente
    - Appelle Edge Function send-email-ionos
    - Gère les erreurs et les retry
  
  2. Cron
    - Toutes les minutes
    - Traite 20 emails max par batch
*/

-- Fonction pour traiter la queue d'emails
...
-- Créer le cron pour traiter la queue
DO $$
BEGIN
  -- Supprimer l'ancien s'il existe
  PERFORM cron.unschedule('process-email-queue-simple');
EXCEPTION WHEN OTHERS THEN
  NULL;
END $$;
...
GRANT EXECUTE ON FUNCTION process_email_queue_simple TO service_role;

COMMENT ON FUNCTION process_email_queue_simple IS 'Traite la queue d''emails en appelant l''Edge Function IONOS. Appelé par cron toutes les minutes.';
