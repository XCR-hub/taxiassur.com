/*
  # Fix get_lead_by_token - Colonne validated_at au lieu de validated - 20 février 2026

  ## Problème identifié

  La fonction utilise `validated = true` mais la colonne n'existe pas.
  La bonne colonne est `validated_at` (timestamp).

  ## Solution

  Un document est validé si `validated_at IS NOT NULL` OU `status = 'validated'`
*/
...
BEGIN
  ...
  IF v_lead_id IS NOT NULL THEN
    -- Compter les TYPES de documents distincts uploadés
    ...
    -- Compter les TYPES de documents distincts VALIDÉS (validated_at IS NOT NULL)
    SELECT COUNT(DISTINCT document_type)
    INTO v_validated_docs
    FROM crm_lead_documents
    WHERE lead_id = v_lead_id
      AND (validated_at IS NOT NULL OR status = 'validated')
      AND document_type IN (
        'licence_taxi',
        'permis_conduire',
        'piece_identite',
...
'Récupère un lead par son access_token avec compteurs de documents (corrigé validated_at)';
