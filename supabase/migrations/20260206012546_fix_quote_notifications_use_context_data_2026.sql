/*
  # Fix quote notifications to use context_data for compatibility

  Met à jour les fonctions pour utiliser context_data au lieu de metadata,
  car le composant RealtimeNotifications lit context_data.action_url.
  
  Utilise aussi la colonne action_url directe pour une meilleure compatibilité.
*/

-- Fonction pour valider un devis (corrigée)
...
BEGIN
  ...
  v_action_url := '/backoffice/crm-killer/lead/' || v_lead_id::text;
  ...
  INSERT INTO crm_event_notifications (
    ...
    format('%s a accepté le devis de %s (%s €). Cliquez pour voir le lead.', 
      v_lead_name, 
      v_company_name,
...
-- Fonction pour refuser un devis (corrigée)
...
BEGIN
  ...
  INSERT INTO crm_event_notifications (
    ...
    format('%s a refusé le devis de %s%s. Cliquez pour voir le lead.', 
      v_lead_name, 
      v_company_name,
      CASE 
...
COMMENT ON FUNCTION validate_quote_by_token(uuid, text) IS 'Valide un devis depuis l''espace prospect, notifie le commercial avec lien cliquable dans context_data';
COMMENT ON FUNCTION refuse_quote_by_token(uuid, text, text) IS 'Refuse un devis depuis l''espace prospect, notifie le commercial avec lien cliquable dans context_data';
