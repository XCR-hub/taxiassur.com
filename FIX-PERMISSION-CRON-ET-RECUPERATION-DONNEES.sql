...
-- Politique : Seuls les utilisateurs authentifiés peuvent lire la config
CREATE POLICY IF NOT EXISTS "Service role can read cron config"
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'total_news', (SELECT COUNT(*) FROM news WHERE published = true),
...
-- ============================================================================
-- ÉTAPE 3 : PERMISSIONS ET ACCÈS ANONYME
-- ============================================================================
...
GRANT EXECUTE ON FUNCTION public.get_leads TO authenticated;
...
SELECT '✓ Configuration cron' as test, get_cron_config('supabase_url') as result
UNION ALL
SELECT '✓ Blog posts', COUNT(*)::text FROM get_blog_posts(10, 0)
UNION ALL
SELECT '✓ FAQs', COUNT(*)::text FROM get_faqs()
UNION ALL
SELECT '✓ Stats', (get_dashboard_stats()->>'total_blog_posts')::text;
