...
const config = {
  ...
  imapHost: env.IMAP_HOST || env.STALWART_IMAP_HOST || '192.168.1.91',
  imapPort: positiveInt(env.IMAP_PORT || env.STALWART_IMAP_PORT, 993, 65535),
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/v1/admin/dashboard') return await adminDashboard(req, res, origin, requestId);
...
async function adminLeadQuotesWorkspace(req,res,origin,requestId,leadId){
  ...
  let quotes=await recordsWhere('lead_company_quotes','lead_id',leadId);
  ...
  quotes=quotes.map(function(row){return Object.assign({},row,{company:allCompanies.find(function(company){return String(company.id)===String(row.company_id||row.insurance_company_id);})||null});}).sort(function(a,b){return String(a.created_at||'').localeCompare(String(b.created_at||''));});
...
async function adminLeadQuotePatch(req,res,origin,requestId,leadId,quoteId){
  ...
  const quote=parseJsonLine(await runPsql(`UPDATE taxiassur.records SET data=data||${quoteLiteral(JSON.stringify(updates))}::jsonb,updated_at=now(),revision=revision+1 WHERE collection='lead_company_quotes' AND record_id=${quoteLiteral(quoteId)} AND data->>'lead_id'=${quoteLiteral(leadId)} RETURNING data::text;`));
...
async function adminQuoteContext(leadId,quoteId){return parseJsonLine(await runPsql(`SELECT jsonb_build_object('quote',q.data,'lead',l.data,'company',c.data)::text FROM taxiassur.records q JOIN taxiassur.records l ON l.collection='crm_leads' AND l.record_id=${quoteLiteral(leadId)} LEFT JOIN taxiassur.records c ON c.collection='insurance_companies' AND c.record_id=COALESCE(q.data->>'company_id',q.data->>'insurance_company_id') WHERE q.collection='lead_company_quotes' AND q.record_id=${quoteLiteral(quoteId)} AND q.data->>'lead_id'=${quoteLiteral(leadId)} LIMIT 1;`));}
async function adminLeadQuoteDocumentUpload(req,res,origin,requestId,leadId,quoteId){
 ...
 try{await runPsql(`BEGIN;INSERT INTO taxiassur.file_objects(id,owner_type,owner_id,document_type,original_name,storage_path,mime_type,size_bytes,sha256_hex,scan_status,scan_engine,scan_checked_at,status) VALUES(${quoteLiteral(fileId)}::uuid,'admin',${quoteLiteral(leadId)},${quoteLiteral(kind==='rc_pro'?'devis_rc_pro':'devis')},${quoteLiteral(name)},${quoteLiteral(relativePath)},'application/pdf',${upload.size},${quoteLiteral(upload.sha256)},'clean','clamav',now(),'validated');UPDATE taxiassur.records SET data=data||${quoteLiteral(JSON.stringify(updates))}::jsonb,updated_at=now(),revision=revision+1 WHERE collection='lead_company_quotes' AND record_id=${quoteLiteral(quoteId)} AND data->>'lead_id'=${quoteLiteral(leadId)};INSERT INTO taxiassur.audit_events(actor_type,actor_id,action,target_type,target_id,request_id) VALUES('admin',${quoteLiteral(session.sub)},'lead_quote_document_uploaded','lead_company_quote',${quoteLiteral(quoteId)},${quoteLiteral(requestId)}::uuid);COMMIT;`);}catch(error){safeUnlink(finalPath);throw error;}if(previous.startsWith(`${leadId}/quotes/`)&&previous!==relativePath)safeUnlink(safeStoragePath(previous));return json(res,origin,201,{ok:true,path:relativePath,quote:Object.assign({},context.quote,updates)},requestId);
}

async function adminLeadQuoteDocumentDownload(req,res,origin,requestId,leadId,quoteId){const session=await verifiedAdminSession(req);if(!session)return json(res,origin,401,{ok:false,error:'invalid_session'},requestId);const context=await adminQuoteContext(leadId,quoteId);if(!context)return json(res,origin,404,{ok:false,error:'not_found'},requestId);const kind=new URL(req.url||'/', 'http://localhost').searchParams.get('kind')==='rc_pro'?'rc_pro':'quote',storagePath=String(kind==='rc_pro'?context.quote.rc_pro_addon_file_url:(context.quote.quote_file_url||context.quote.quote_pdf_url)||'');if(!storagePath)return json(res,origin,404,{ok:false,error:'not_found'},requestId);const objectPath=storagePath.startsWith(`${leadId}/quotes/`)?storagePath:storageObjectPath(storagePath,'contract-documents');const filePath=objectPath.startsWith(`${leadId}/quotes/`)?safeStoragePath(objectPath):safeLegacyStoragePath('contract-documents',objectPath);if(!existsSync(filePath))return json(res,origin,404,{ok:false,error:'file_missing'},requestId);res.writeHead(200,responseHeaders(origin,requestId,{'Content-Type':'application/pdf','Content-Length':String(statSync(filePath).size),'Content-Disposition':`inline; filename*=UTF-8''${encodeURIComponent(context.quote.file_name||'devis.pdf')}`,'Cache-Control':'private, no-store'}));createReadStream(filePath).pipe(res);}
async function adminLeadQuoteDocumentDelete(req,res,origin,requestId,leadId,quoteId){const session=await verifiedAdminSession(req);if(!session)return json(res,origin,401,{ok:false,error:'invalid_session'},requestId);const context=await adminQuoteContext(leadId,quoteId);if(!context)return json(res,origin,404,{ok:false,error:'not_found'},requestId);const storagePath=String(context.quote.quote_file_url||context.quote.quote_pdf_url||''),updates={quote_file_url:null,quote_pdf_url:null,file_name:null,status:'pending',quote_status:'pending',submitted_at:null,updated_at:new Date().toISOString()};await runPsql(`BEGIN;UPDATE taxiassur.records SET data=data||${quoteLiteral(JSON.stringify(updates))}::jsonb,updated_at=now(),revision=revision+1 WHERE collection='lead_company_quotes' AND record_id=${quoteLiteral(quoteId)} AND data->>'lead_id'=${quoteLiteral(leadId)};INSERT INTO taxiassur.audit_events(actor_type,actor_id,action,target_type,target_id,request_id)VALUES('admin',${quoteLiteral(session.sub)},'lead_quote_document_deleted','lead_company_quote',${quoteLiteral(quoteId)},${quoteLiteral(requestId)}::uuid);COMMIT;`);if(storagePath.startsWith(`${leadId}/quotes/`))safeUnlink(safeStoragePath(storagePath));return json(res,origin,200,{ok:true},requestId);}
...
async function adminLeadTimeline(req,res,origin,requestId,leadId){
  ...
  if(req.method==='POST'){
    ...
    await runPsql("BEGIN;INSERT INTO taxiassur.records(collection,record_id,data,origin)VALUES('crm_interactions',"+quoteLiteral(id)+","+quoteLiteral(JSON.stringify(note))+"::jsonb,'admin');INSERT INTO taxiassur.audit_events(actor_type,actor_id,action,target_type,target_id,request_id)VALUES('admin',"+quoteLiteral(session.sub)+",'crm_note_added','crm_lead',"+quoteLiteral(leadId)+","+quoteLiteral(requestId)+"::uuid);COMMIT;");
    return json(res,origin,201,{ok:true,note},requestId);
  }
...
async function adminInboxWorkflow(req,res,origin,requestId){
  ...
  if(action==='create_lead'){const emailId=String(body.email_id||''),source=parseJsonLine(await runPsql(`SELECT data::text FROM taxiassur.records WHERE collection='email_messages' AND record_id=${quoteLiteral(emailId)} LIMIT 1;`));if(!source)return json(res,origin,404,{ok:false,error:'email_not_found'},requestId);const email=String(body.email||source.from_email||'').trim().toLowerCase(),name=String(body.name||source.from_name||'').trim(),parts=name.split(/\s+/),id=randomUUID(),now=new Date().toISOString(),lead={id,first_name:parts.shift()||'',last_name:parts.join(' '),full_name:name,email,phone:String(body.phone||''),city:String(body.city||''),status:'NOUVEAU_LEAD',pipeline_stage:'NOUVEAU_LEAD',source:'email',notes:String(body.notes||'').slice(0,2000),created_at:now,updated_at:now};if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))return json(res,origin,400,{ok:false,error:'invalid_email'},requestId);await runPsql(`INSERT INTO taxiassur.records(collection,record_id,data,origin)VALUES('crm_leads',${quoteLiteral(id)},${quoteLiteral(JSON.stringify(lead))}::jsonb,'admin');`);const linked=await linkInboxHistory(id,String(source.from_email||email).toLowerCase());return json(res,origin,201,{ok:true,lead,...linked},requestId);}
  if(action==='reply'){const emailId=String(body.email_id||''),source=parseJsonLine(await runPsql(`SELECT data::text FROM taxiassur.records WHERE collection='email_messages' AND record_id=${quoteLiteral(emailId)} LIMIT 1;`)),content=String(body.content||'').trim().slice(0,10000);if(!source||!content)return json(res,origin,400,{ok:false,error:'invalid_reply'},requestId);const id=randomUUID(),now=new Date().toISOString(),recipient=String(source.from_email||'').trim().toLowerCase(),subject=('Re: '+String(source.subject||'')).slice(0,250),mail={id,recipient,subject,body:content,status:'pending',attempts:0,next_attempt_at:now,created_at:now,lead_id:source.lead_id||null};await runPsql(`BEGIN;INSERT INTO taxiassur.records(collection,record_id,data,origin)VALUES('native_email_outbox',${quoteLiteral(id)},${quoteLiteral(JSON.stringify(mail))}::jsonb,'admin');INSERT INTO taxiassur.audit_events(actor_type,actor_id,action,target_type,target_id,request_id)VALUES('admin',${quoteLiteral(session.sub)},'inbox_reply_queued','email_message',${quoteLiteral(emailId)},${quoteLiteral(requestId)}::uuid);COMMIT;`);return json(res,origin,202,{ok:true,queued:true},requestId);}
...
async function adminInbox(req,res,origin,requestId,url){
  ...
  if(req.method==='GET'){
    ...
    const all=(await recordsAll('email_messages')).sort((a,b)=>Date.parse(String(b.received_at||b.created_at||''))-Date.parse(String(a.received_at||a.created_at||'')));
    const active=row=>!row.email_status||row.email_status==='active';
    const stats={total:all.filter(active).length,unread:all.filter(row=>active(row)&&!row.is_read).length,leads:all.filter(row=>active(row)&&row.lead_id).length,starred:all.filter(row=>active(row)&&row.is_starred).length,archived:all.filter(row=>row.email_status==='archived').length,mails:all.filter(row=>active(row)&&row.classification==='non_lead').length};
    const matches=row=>!search||[row.subject,row.from_email,row.body_text].some(value=>String(value||'').toLowerCase().includes(search));
    const messages=all.filter(row=>matches(row)&&(filter==='archived'?row.email_status==='archived':active(row)&&(filter==='unread'?!row.is_read:filter==='starred'?row.is_starred:filter==='leads'?Boolean(row.lead_id):filter==='mails'?row.classification==='non_lead':true))).slice(0,500);
    return json(res,origin,200,{ok:true,messages,stats},requestId);
  }
  ...
  if(!ids.length||!['mark_read','mark_all_read','star','archive','delete','classify_non_lead','assign'].includes(action))return json(res,origin,400,{ok:false,error:'invalid_inbox_action'},requestId);
...
function readableMailBody(raw){let body=String(raw).split(/\r?\n\r?\n/).slice(1).join('\n\n');body=body.replace(/<style[\s\S]*?<\/style>/gi,' ').replace(/<script[\s\S]*?<\/script>/gi,' ').replace(/<br\s*\/?\s*>/gi,'\n').replace(/<\/p>/gi,'\n').replace(/<[^>]+>/g,' ').replace(/&nbsp;/gi,' ').replace(/&amp;/gi,'&').replace(/&lt;/gi,'<').replace(/&gt;/gi,'>').replace(/=\r?\n/g,'').replace(/=([0-9a-f]{2})/gi,(_m,h)=>String.fromCharCode(parseInt(h,16))).replace(/\r/g,'').replace(/[ \t]+\n/g,'\n').replace(/\n{3,}/g,'\n\n').trim();return body.slice(0,50000);}
function stableMailId(uid){const h=createHash('sha256').update(config.smtpUser.toLowerCase()+':'+uid).digest('hex').slice(0,32).split('');h[12]='4';h[16]=['8','9','a','b'][parseInt(h[16],16)%4];return `${h.slice(0,8).join('')}-${h.slice(8,12).join('')}-${h.slice(12,16).join('')}-${h.slice(16,20).join('')}-${h.slice(20).join('')}`;}
function imapSync(){return new Promise((resolve,reject)=>{const socket=tls.connect({host:config.imapHost,port:config.imapPort,rejectUnauthorized:false}),timeout=setTimeout(()=>{socket.destroy();reject(new Error('imap_timeout'));},45000);let buffer=Buffer.alloc(0),tagNumber=0,pending=null,greeted=false;const finishError=(error)=>{clearTimeout(timeout);try{socket.destroy();}catch{}reject(error);};const command=(text)=>new Promise((res,rej)=>{const tag='x'+(++tagNumber);pending={tag,res,rej,start:buffer.length};socket.write(`${tag} ${text}\r\n`);});socket.on('data',chunk=>{buffer=Buffer.concat([buffer,chunk]);if(!greeted&&buffer.includes(Buffer.from('\r\n'))){greeted=true;run().catch(finishError);return;}if(pending){const text=buffer.toString('latin1');const match=text.match(new RegExp('(?:^|\\r\\n)'+pending.tag+' (OK|NO|BAD)[^\\r\\n]*\\r\\n','i'));if(match){const current=pending;pending=null;if(match[1].toUpperCase()==='OK')current.res(buffer);else current.rej(new Error('imap_command_failed'));}}});socket.on('error',finishError);async function run(){await command(`LOGIN "${String(config.smtpUser).replace(/["\\]/g,'')}" "${String(config.smtpPassword).replace(/["\\]/g,'')}"`);buffer=Buffer.alloc(0);await command('SELECT INBOX');buffer=Buffer.alloc(0);const search=await command('UID SEARCH ALL');const searchText=search.toString('latin1'),line=searchText.match(/\* SEARCH([^\r\n]*)/i)?.[1]||'',uids=line.trim().split(/\s+/).filter(x=>/^\d+$/.test(x)).slice(-100);if(!uids.length){clearTimeout(timeout);socket.end();return resolve([]);}buffer=Buffer.alloc(0);const fetched=await command(`UID FETCH ${uids.join(',')} (UID FLAGS INTERNALDATE BODY.PEEK[])`);const rows=[];let offset=0;while(offset<fetched.length){const marker=fetched.indexOf(Buffer.from('\r\n'),offset);if(marker<0)break;const prefix=fetched.subarray(offset,marker).toString('latin1'),literal=prefix.match(/\{(\d+)\}$/),uid=prefix.match(/UID (\d+)/i);if(literal&&uid){const size=Number(literal[1]),start=marker+2,end=start+size;if(end>fetched.length)break;rows.push({uid:uid[1],flags:prefix.match(/FLAGS \(([^)]*)\)/i)?.[1]||'',raw:fetched.subarray(start,end).toString('utf8')});offset=end;continue;}offset=marker+2;}clearTimeout(timeout);socket.write(`x${++tagNumber} LOGOUT\r\n`);socket.end();resolve(rows);}});}
async function adminInboxSync(req,res,origin,requestId){const session=await verifiedAdminSession(req);if(!session)return json(res,origin,401,{ok:false,error:'invalid_session'},requestId);if(!config.smtpUser||!config.smtpPassword)return json(res,origin,503,{ok:false,error:'mailbox_not_configured'},requestId);let rows;try{rows=await imapSync();}catch(error){console.error('[inbox-sync]',error?.message||error);return json(res,origin,502,{ok:false,error:'imap_sync_failed'},requestId);}const statements=[];for(const item of rows){const id=stableMailId(item.uid),fromRaw=mailHeader(item.raw,'From'),toRaw=mailHeader(item.raw,'To'),subject=mailHeader(item.raw,'Subject')||'(Sans objet)',dateRaw=mailHeader(item.raw,'Date'),receivedAt=Number.isFinite(Date.parse(dateRaw))?new Date(dateRaw).toISOString():new Date().toISOString(),record={id,mailbox_uid:item.uid,message_id:mailHeader(item.raw,'Message-ID'),from_email:mailAddress(fromRaw),from_name:decodeMailHeader(fromRaw.replace(/<[^>]+>/g,'')).replace(/^"|"$/g,'').trim(),to_emails:toRaw.split(',').map(mailAddress).filter(Boolean),subject,body_text:readableMailBody(item.raw),direction:mailAddress(fromRaw)===config.smtpUser.toLowerCase()?'outbound':'inbound',is_read:/\\Seen/i.test(item.flags),is_starred:/\\Flagged/i.test(item.flags),email_status:'active',received_at:receivedAt,created_at:receivedAt,synced_at:new Date().toISOString(),sync_source:'stalwart-imap'};statements.push(`INSERT INTO taxiassur.records(collection,record_id,data,origin)VALUES('email_messages',${quoteLiteral(id)},${quoteLiteral(JSON.stringify(record))}::jsonb,'imap') ON CONFLICT(collection,record_id) DO UPDATE SET data=taxiassur.records.data||EXCLUDED.data,updated_at=now(),revision=taxiassur.records.revision+1;`);}const imported=rows.length;statements.push(`INSERT INTO taxiassur.audit_events(actor_type,actor_id,action,target_type,target_id,request_id,metadata)VALUES('admin',${quoteLiteral(session.sub)},'inbox_imap_synced','mailbox',${quoteLiteral(config.smtpUser)},${quoteLiteral(requestId)}::uuid,${quoteLiteral(JSON.stringify({retrieved:rows.length,imported}))}::jsonb);`);await runPsql('BEGIN;'+statements.join('')+'COMMIT;');return json(res,origin,200,{ok:true,success:true,stats:{emails_retrieved:rows.length,emails_imported:imported}},requestId);}
async function adminPipelineNotifications(req,res,origin,requestId){
  ...
  return json(res,origin,200,{ok:true,notifications},requestId);
}
...
async function adminDocumentOpen(req,res,origin,requestId){
  ...
  const filePath=safeLegacyStoragePath(bucket,storagePath);
...
async function adminDocumentPatch(req,res,origin,requestId,documentId){
  ...
  return json(res,origin,200,{ok:true,document:document,email_queued:status==='rejected'},requestId);
}
...
async function adminDocumentDownload(req,res,origin,requestId,documentId){
  if(!await verifiedAdminSession(req))return json(res,origin,401,{ok:false,error:'invalid_session'},requestId);
  const row=parseJsonLine(await runPsql(`SELECT data::text FROM taxiassur.records WHERE collection IN ('prospect_documents','crm_lead_documents') AND record_id=${quoteLiteral(documentId)} LIMIT 1;`));if(!row)return json(res,origin,404,{ok:false,error:'not_found'},requestId);
...
async function createPublicLead(req, res, origin, requestId) {
  ...
  await runPsql(`BEGIN; INSERT INTO taxiassur.records(collection,record_id,data,origin) VALUES('crm_leads',${quoteLiteral(id)},${quoteLiteral(JSON.stringify(lead))}::jsonb,'local'),('native_email_outbox',${quoteLiteral(outboxId)},${quoteLiteral(JSON.stringify(outbox))}::jsonb,'local'),('native_email_outbox',${quoteLiteral(prospectOutboxId)},${quoteLiteral(JSON.stringify(prospectOutbox))}::jsonb,'local'),('conversion_events',${quoteLiteral(conversionId)},${quoteLiteral(JSON.stringify(conversion))}::jsonb,'server'); INSERT INTO taxiassur.audit_events(actor_type,actor_id,action,target_type,target_id,request_id,metadata) VALUES('public',${quoteLiteral(email)},'lead_created','crm_lead',${quoteLiteral(id)},${quoteLiteral(requestId)}::uuid,${quoteLiteral(JSON.stringify({ source: lead.source, service, landing_page: acquisition.landing_page, prospect_email_queued: true }))}::jsonb); COMMIT;`);
...
async function adminDocumentWorkspaceUpload(req,res,origin,requestId,leadId){
  ...
  if(!['pdf','jpg','png','webp'].includes(extension)||!allowedDocumentTypes.has(documentType)||(documentType==='custom'&&!customLabel)||!name||!Number.isInteger(size)||size<1||size>maxUploadBytes)return drainAndJson(req,res,origin,400,{ok:false,error:'invalid_document_upload'},requestId);
...
async function adminDocumentWorkspace(req,res,origin,requestId,leadId){
  ...
  if(req.method==='GET'){
    ...
    const attachments=parseJsonLine(await runPsql(sql))||[];return json(res,origin,200,{ok:true,attachments,unimported_attachments:[]},requestId);
  }
  const body=await readJsonBody(req),action=String(body.action||'');
  if(action==='move'){
  ...
  const match=/^([0-9a-f-]{36}):([0-9]{1,5})$/i.exec(String(body.attachment_id||''));const documentType=String(body.document_type||'').trim().toLowerCase(),customLabel=String(body.custom_label||'').normalize('NFKC').trim().slice(0,160);
  if(!match||!allowedDocumentTypes.has(documentType)||(documentType==='custom'&&!customLabel))return json(res,origin,400,{ok:false,error:'invalid_classification'},requestId);
...
async function uploadAdminContractDocument(req, res, origin, requestId, leadId) {
  ...
  try {
    await runPsql(`BEGIN;
      ...
      VALUES (${quoteLiteral(fileId)}::uuid, 'admin', ${quoteLiteral(leadId)}, ${quoteLiteral(documentType)}, ${quoteLiteral(originalName)}, ${quoteLiteral(relativePath)}, 'application/pdf', ${upload.size}, ${quoteLiteral(upload.sha256)}, 'clean', 'clamav', now(), 'validated');
  ...
  return json(res, origin, 201, { ok: true, document, field, path: relativePath }, requestId);
}
