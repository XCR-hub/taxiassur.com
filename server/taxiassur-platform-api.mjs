...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/v1/admin/email-account') return await adminEmailAccount(req,res,origin,requestId);
    ...
    if (req.method === 'GET' && url.pathname === '/v1/admin/pipeline-dashboard') return await adminPipelineDashboard(req,res,origin,requestId,url);
...
async function adminEmailAccount(req,res,origin,requestId){if(!await verifiedAdminSession(req))return json(res,origin,401,{ok:false,error:'invalid_session'},requestId);const lastSync=String(await runPsql("SELECT COALESCE(max(data->>'synced_at'),'') FROM taxiassur.records WHERE collection='email_messages';")).trim()||null;return json(res,origin,200,{ok:true,account:{id:'native-imap',email:config.imapUser,imap_host:config.imapHost,imap_port:config.imapPort,imap_username:config.imapUser,smtp_host:config.smtpHost,smtp_port:config.smtpPort,is_active:Boolean(config.imapUser&&config.imapPassword),password_configured:Boolean(config.imapPassword),last_sync_at:lastSync}},requestId);}
async function adminInboxWorkflow(req,res,origin,requestId){
  ...
  if(action==='legacy_auto_create_leads_disabled')return json(res,origin,403,{ok:false,error:'automatic_lead_creation_disabled'},requestId);
...
async function autoProcessInboxLeads(){
  ...
  for(const source of candidates){
    const email=String(source.from_email||'').trim().toLowerCase();
  ...
  return {created:0,linked,ignored};
}
...
async function adminPipelineDashboard(req,res,origin,requestId,url){
  if(!await verifiedAdminSession(req))return json(res,origin,401,{ok:false,error:'invalid_session'},requestId);
  const leadId=String(url.searchParams.get('lead_id')||'');
  const [allLeads,documents,interactions,emails,quotes,contracts,reminders]=await Promise.all([recordsAll('crm_leads'),recordsAll('crm_lead_documents'),recordsAll('crm_interactions'),recordsAll('email_messages'),recordsAll('lead_company_quotes'),recordsAll('lead_contracts'),recordsAll('crm_reminders')]);
  const leads=allLeads.filter(x=>!x.deleted_at&&x.is_archived!==true);
  if(leadId){
    if(!leads.some(x=>String(x.id)===leadId))return json(res,origin,404,{ok:false,error:'lead_not_found'},requestId);
    const communications=[...interactions,...emails].filter(x=>String(x.lead_id)===leadId).sort((a,b)=>Date.parse(String(b.created_at||b.received_at||''))-Date.parse(String(a.created_at||a.received_at||'')));
    const leadDocuments=documents.filter(x=>String(x.lead_id)===leadId);
    return json(res,origin,200,{ok:true,communications,documents:leadDocuments},requestId);
  }
  const stageOf=x=>String(x.current_stage_key||x.current_stage||x.pipeline_stage||x.status||'nouveau_lead').toLowerCase();
  const categoryOf=stage=>/client|signe/.test(stage)?'client':/contrat/.test(stage)?'contrat':/paiement/.test(stage)?'paiement':/devis|quote/.test(stage)?'devis':/document|information|verification/.test(stage)?'qualification':'acquisition';
  const now=Date.now(),today=new Date().toISOString().slice(0,10),byStage={};
  const view=leads.map(lead=>{const id=String(lead.id),stage=stageOf(lead),docs=documents.filter(x=>String(x.lead_id)===id),comms=[...interactions,...emails].filter(x=>String(x.lead_id)===id),pending=reminders.some(x=>String(x.lead_id)===id&&!['completed','cancelled'].includes(String(x.status||''))),created=Date.parse(String(lead.stage_updated_at||lead.updated_at||lead.created_at||''));byStage[stage]=(byStage[stage]||0)+1;return {lead_id:id,lead_name:lead.full_name||String(`${lead.first_name||lead.prenom||''} ${lead.last_name||lead.nom||lead.name||''}`).trim()||lead.email||'Lead sans nom',lead_email:lead.email||'',lead_phone:lead.phone||lead.telephone||'',lead_status:lead.status||stage,current_stage:stage,stage_category:categoryOf(stage),documents_complete:Boolean(lead.documents_complete)||docs.length>0&&docs.every(x=>['validated','approved'].includes(String(x.status||''))),info_complete:Boolean(lead.info_complete),missing_documents:Array.isArray(lead.missing_documents)?lead.missing_documents:[],missing_info:Array.isArray(lead.missing_info)?lead.missing_info:[],days_in_current_stage:Number.isFinite(created)?Math.max(0,Math.floor((now-created)/86400000)):0,total_communications:comms.length,last_communication_date:comms.map(x=>x.created_at||x.received_at).filter(Boolean).sort().at(-1)||null,has_pending_reminders:pending,created_at:lead.created_at||null};});
  const activeClients=view.filter(x=>['client_actif','active_client','signed','contrat_signe'].includes(x.current_stage)).length,acceptedQuotes=quotes.filter(x=>['accepted','validated','accepte'].includes(String(x.status||'').toLowerCase())),sentToday=[...interactions,...emails].filter(x=>String(x.created_at||x.sent_at||'').startsWith(today)&&x.direction!=='inbound').length;
  const statistics={total_leads:view.length,new_leads_today:view.filter(x=>String(x.created_at||'').startsWith(today)).length,documents_pending:documents.filter(x=>['uploaded','pending','pending_validation'].includes(String(x.status||''))).length,quotes_pending:quotes.filter(x=>['sent','pending','pending_response'].includes(String(x.status||''))).length,active_clients:activeClients,conversion_rate_percent:view.length?Math.round(activeClients*1000/view.length)/10:0,leads_by_stage:byStage,average_time_to_quote_days:0,reminders_scheduled_today:reminders.filter(x=>String(x.scheduled_at||x.reminder_date||'').startsWith(today)).length,communications_sent_today:sentToday,payments_pending:contracts.filter(x=>['pending','required'].includes(String(x.down_payment_status||x.payment_status||''))).length,contracts_pending_signature:contracts.filter(x=>['pending','sent'].includes(String(x.status||''))).length,quotes_accepted:acceptedQuotes.length};
  return json(res,origin,200,{ok:true,leads:view.sort((a,b)=>String(b.created_at||'').localeCompare(String(a.created_at||''))),statistics},requestId);
}
...
async function adminAutomationDashboard(req,res,origin,requestId){
  ...
  if(req.method==='POST'&&action==='diagnostic'){const name=String(body.name||'').slice(0,120),allowed=new Set(['pipeline-ia-orchestrator','master-ai-decision-engine','crm-automation-engine','lead-scoring-ai','email-automation-engine','content-auto-scheduler','rss-parser','linkedin-scraper','news-aggregator-master','news-digest-generator','news-email-alerts','generate-seo-content','backlink-auto-outreach','social-media-publisher']);if(!allowed.has(name))return json(res,origin,400,{ok:false,error:'unsupported_automation'},requestId);const id=randomUUID(),now=new Date().toISOString(),log={id,action_type:name,status:'success',executed_at:now,execution_time_ms:0,details:{mode:'diagnostic',actor:session.sub}};await runPsql(`INSERT INTO taxiassur.records(collection,record_id,data,origin)VALUES('crm_automation_history',${quoteLiteral(id)},${quoteLiteral(JSON.stringify(log))}::jsonb,'admin');`);return json(res,origin,200,{ok:true,success:true,message:'Diagnostic natif termine; aucun envoi reel declenche'},requestId);}
  return json(res,origin,400,{ok:false,error:'invalid_action'},requestId);
}
async function adminAutomationCenter(req,res,origin,requestId){
  const session=await verifiedAdminSession(req);if(!session)return json(res,origin,401,{ok:false,error:'invalid_session'},requestId);
  if(req.method==='GET'){const configs=await recordsAll('cron_jobs_config'),logs=(await recordsAll('automation_logs')).sort((a,b)=>Date.parse(String(b.created_at||''))-Date.parse(String(a.created_at||''))).slice(0,20),automations=configs.map(x=>{const name=x.name||x.job_name||x.id,related=logs.filter(l=>(l.automation_name||l.job_name)===name),successful=related.filter(l=>l.status==='success').length,failed=related.filter(l=>l.status==='error').length,total=Number(x.total_runs||related.length);return {...x,name,is_enabled:x.is_enabled??x.is_active??false,total_runs:total,successful_runs:Number(x.successful_runs||successful),failed_runs:Number(x.failed_runs||failed),success_rate:total?Math.round(Number(x.successful_runs||successful)*100/total):0};});return json(res,origin,200,{ok:true,automations,logs},requestId);}
  const body=await readJsonBody(req),action=String(body.action||'');
  if(action==='toggle'){const name=String(body.name||'').slice(0,120),enabled=body.enabled===true,item=parseJsonLine(await runPsql(`UPDATE taxiassur.records SET data=data||jsonb_build_object('is_enabled',${enabled},'is_active',${enabled},'updated_at',now()::text),updated_at=now(),revision=revision+1 WHERE collection='cron_jobs_config' AND COALESCE(data->>'name',data->>'job_name',record_id)=${quoteLiteral(name)} RETURNING data::text;`));return item?json(res,origin,200,{ok:true,item},requestId):json(res,origin,404,{ok:false,error:'not_found'},requestId);}
  ...
  if(action==='test'){const name=String(body.name||'').slice(0,120),known=parseJsonLine(await runPsql(`SELECT data::text FROM taxiassur.records WHERE collection='cron_jobs_config' AND COALESCE(data->>'name',data->>'job_name',record_id)=${quoteLiteral(name)} LIMIT 1;`));if(!known)return json(res,origin,404,{ok:false,error:'not_found'},requestId);const id=randomUUID(),now=new Date().toISOString(),log={id,automation_name:name,status:'success',message:'Diagnostic natif termine; configuration valide',duration_ms:0,created_at:now,created_by:session.sub};await runPsql(`INSERT INTO taxiassur.records(collection,record_id,data,origin)VALUES('automation_logs',${quoteLiteral(id)},${quoteLiteral(JSON.stringify(log))}::jsonb,'admin');`);return json(res,origin,200,{ok:true,success:true,duration_ms:0},requestId);}
  return json(res,origin,400,{ok:false,error:'invalid_action'},requestId);
}
...
async function downloadProspectFinalDocument(req, res, origin, requestId, documentId) {
  ...
  const declaredBucket = String(row.bucket || '').toLowerCase();
