...
const QuotesManager: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    const submitted = quotes.filter((q) =>
      q.status === "quote_submitted"
    ).length;
    const withFile =
      quotes.filter((q) => q.quote_file_url || q.quote_pdf_url).length;
  ...
  const filteredQuotes = useMemo(() => {
    return quotes.filter((q) => {
      ...
      if (searchTerm) {
        ...
        const name = `${q.lead?.first_name || ""} ${q.lead?.last_name || ""}`
          .toLowerCase();
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-400 text-sm">
              {stats.total} devis pour {stats.uniqueLeads} prospects
            </p>
          </div>
        </div>
          ...
          : (
            <div className="space-y-3">
              {groupedByLead.map(([leadId, leadQuotes]) => {
                ...
                const validatedCount = leadQuotes.filter((q) =>
                  q.status === "validated"
                ).length;
                const refusedCount =
                  leadQuotes.filter((q) => q.status === "refused").length;
                const pendingCount = leadQuotes.filter((q) =>
                  q.status === "pending"
                ).length;
                                ...
                                <div>
                                  ...
                                  <div className="flex items-center gap-3 mt-1 text-xs text-gray-500">
                                    ...
                                    {quote.last_sent_at && (
                                      <span>
                                        Envoy\u00e9 le{" "}
                                        {formatDate(quote.last_sent_at)}
                                      </span>
                                    )}
                                    {quote.quote_accepted_at && (
                                      <span className="text-emerald-400">
                                        Accept\u00e9 le{" "}
                                        {formatDate(quote.quote_accepted_at)}
                                      </span>
                                    )}
                                    {quote.refused_at && (
                                      <span className="text-red-400">
                                        Refus\u00e9 le{" "}
                                        {formatDate(quote.refused_at)}
                                      </span>
                                    )}
                                  </div>
...
function StatCard(
  { label, value, icon: Icon, color, textColor = "text-white" }: {
    label: string;
    value: string | number;
...
function QuoteDetailModal(
  { quote, onClose }: { quote: Quote; onClose: () => void },
) {
  const formatDateTime = (date: string | null) => {
    if (!date) return "-";
    return new Date(date).toLocaleDateString("fr-FR", {
  ...
  return (
    ...
    >
      <div
        className="bg-gray-800 border border-gray-700 rounded-2xl w-full max-w-lg max-h-[80vh] overflow-auto"
        onClick={(e) => e.stopPropagation()}
      >
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="border-t border-gray-700 pt-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              <TimelineRow
                label="Soumis"
                date={formatDateTime(quote.submitted_at)}
              />
              <TimelineRow
                label="Envoy\u00e9"
                date={formatDateTime(quote.last_sent_at)}
              />
              <TimelineRow
                label="Envoy\u00e9 au client"
                date={formatDateTime(quote.sent_to_client_at)}
              />
              <TimelineRow
                label="Accept\u00e9"
                date={formatDateTime(quote.quote_accepted_at)}
                highlight="emerald"
              />
              <TimelineRow
                label="Valid\u00e9"
                date={formatDateTime(quote.validated_at)}
                highlight="emerald"
              />
              <TimelineRow
                label="Refus\u00e9"
                date={formatDateTime(quote.refused_at)}
                highlight="red"
              />
            </div>
          </div>
          ...
          {(quote.quote_file_url || quote.quote_pdf_url) && (
            <div className="border-t border-gray-700 pt-4">
              <SecureDocumentLink
                filePath={quote.quote_pdf_url || quote.quote_file_url || ""}
                source="crm_lead_documents"
                bucket="contract-documents"
                fileName="Devis.pdf"
                mode="download"
                showText
                customText="Télécharger le PDF du devis"
