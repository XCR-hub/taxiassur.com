...
const STATUS_CONFIG: Record<string, { label: string; bg: string; text: string; icon: LucideIcon; step: number }> = {
  ...
  EXPERT_MISSIONED:       { label: 'Expert missionné',           bg: 'bg-blue-50',   text: 'text-blue-700',   icon: User,        step: 2 },
  EXPERTISE_SCHEDULED:    { label: 'Expertise planifiée',        bg: 'bg-cyan-50',   text: 'text-cyan-700',   icon: Calendar,    step: 2 },
  EXPERTISE_DONE:         { label: 'Expertise réalisée',         bg: 'bg-teal-50',   text: 'text-teal-700',   icon: CheckCircle, step: 3 },
  UNDER_REVIEW:           { label: "En cours d'instruction",     bg: 'bg-blue-50',   text: 'text-blue-700',   icon: Clock,       step: 3 },
  INDEMNISATION_PROPOSED: { label: "Proposition d'indemnisation",bg: 'bg-orange-50', text: 'text-orange-700', icon: DollarSign,  step: 4 },
  REPAIR_IN_PROGRESS:     { label: 'Réparation en cours',        bg: 'bg-purple-50', text: 'text-purple-700', icon: Wrench,      step: 3 },
  APPROVED:               { label: 'Approuvé',                   bg: 'bg-green-50',  text: 'text-green-700',  icon: CheckCircle, step: 4 },
  REJECTED:               { label: 'Refusé',                     bg: 'bg-red-50',    text: 'text-red-700',    icon: AlertCircle, step: 5 },
  PAID:                   { label: 'Indemnisé',                  bg: 'bg-green-50',  text: 'text-green-700',  icon: CheckCircle, step: 5 },
  CLOSED:                 { label: 'Clôturé',                    bg: 'bg-gray-50',   text: 'text-gray-600',   icon: CheckCircle, step: 5 },
};
...
function StatusBadge({ status }: { status: string | null }) {
  const cfg = STATUS_CONFIG[status?.toUpperCase() || 'DECLARED'] || STATUS_CONFIG.DECLARED;
  const Icon = cfg.icon;
  return (
...
export default function ClientSinistres() {
  ...
  const activeClaims = claims.filter(c => !['CLOSED', 'REJECTED', 'PAID'].includes((c.claim_status || 'DECLARED').toUpperCase()));
  const closedClaims = claims.filter(c =>  ['CLOSED', 'REJECTED', 'PAID'].includes((c.claim_status || 'DECLARED').toUpperCase()));
  ...
  const renderClaim = (claim: Claim, closed = false) => {
    ...
    const incidentLabel = INCIDENT_TYPES.find(t => t.value === (claim.incident_type || claim.claim_type))?.label || claim.incident_type || claim.claim_type;
    ...
    return (
      <div key={claim.id} className={`bg-white rounded-xl border ${closed ? 'border-gray-100 opacity-80' : 'border-gray-200 shadow-sm'} overflow-hidden`}>
        <div
          className="flex items-center gap-4 p-4 cursor-pointer hover:bg-gray-50 transition-colors"
          onClick={() => setExpandedClaim(isExpanded ? null : claim.id)}
        >
          <div className={`w-10 h-10 ${closed ? 'bg-gray-100' : 'bg-yellow-100'} rounded-xl flex items-center justify-center flex-shrink-0`}>
            <Shield size={18} className={closed ? 'text-gray-400' : 'text-yellow-600'} />
          </div>
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
              <span className="flex items-center gap-1"><Calendar size={10} />{fmtDate(claim.incident_date)}</span>
              {claim.claim_number && <span className="flex items-center gap-1"><FileText size={10} />N° {claim.claim_number}</span>}
              {claim.incident_location && <span className="flex items-center gap-1"><MapPin size={10} />{claim.incident_location}</span>}
            </div>
          </div>
                      ...
                      <div key={ps.step} className="flex items-center flex-1">
                        <div className="flex flex-col items-center flex-1 min-w-0">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center
                            ${done ? 'bg-green-500' : active ? 'bg-yellow-500 ring-2 ring-yellow-200' : 'bg-gray-100'}`}>
                            {done
                              ? <CheckCircle size={14} className="text-white" />
                              : <Icon size={13} className={active ? 'text-white' : 'text-gray-400'} />
                            }
                          </div>
                          <span className={`text-xs mt-1 font-medium text-center leading-tight w-full ${active ? 'text-yellow-700' : done ? 'text-green-600' : 'text-gray-400'}`}>
                            {ps.label}
                          </span>
                        </div>
                        {idx < PIPELINE_STEPS.length - 1 && (
                          <div className={`h-0.5 flex-1 mx-1 mb-5 rounded ${done ? 'bg-green-400' : 'bg-gray-200'}`} />
                        )}
                      </div>
                    );
                  })}
            ...
            <div className="p-5 space-y-5">
              ...
              {claim.client_visible_status && (
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                  <p className="text-xs font-semibold text-blue-700 mb-1">Mise à jour de votre dossier</p>
                  <p className="text-sm text-blue-900 font-medium">{claim.client_visible_status}</p>
                  {claim.client_visible_notes && (
                    <p className="text-xs text-blue-700 mt-2 leading-relaxed">{claim.client_visible_notes}</p>
                  )}
                </div>
              )}
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Description de l'incident</p>
                <p className="text-sm text-gray-700 leading-relaxed">{claim.incident_description}</p>
                <div className="flex flex-wrap gap-3 mt-2">
                  {claim.third_party_involved && <span className="text-xs text-gray-500 flex items-center gap-1"><User size={10} />Tiers impliqué</span>}
                  {claim.police_report_number && <span className="text-xs text-gray-500 flex items-center gap-1"><FileText size={10} />PV n° {claim.police_report_number}</span>}
                </div>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                {hasExpert && (
                  <div className="bg-white border border-gray-200 rounded-xl p-4">
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3 flex items-center gap-1.5">
                      <User size={11} className="text-blue-500" />Expert missionné
                    </p>
                    <div className="space-y-1.5">
                      <InfoRow icon={User} label="Expert" value={[claim.expert_name, claim.expert_company].filter(Boolean).join(' — ')} />
                      <InfoRow icon={Phone} label="Téléphone" value={claim.expert_phone} />
                      <InfoRow icon={Calendar} label="Rendez-vous" value={fmtDateTime(claim.expert_appointment_date)} />
                    </div>
                  </div>
                )}
                ...
                {hasExpertiseGarage && (
                  <div className="bg-white border border-gray-200 rounded-xl p-4">
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3 flex items-center gap-1.5">
                      <Car size={11} className="text-cyan-500" />Garage d'expertise
                    </p>
                    <div className="space-y-1.5">
                      <InfoRow icon={Building2} label="Garage" value={claim.expertise_garage_name} />
                      <InfoRow icon={MapPin} label="Adresse" value={claim.expertise_garage_address} />
                      <InfoRow icon={Phone} label="Téléphone" value={claim.expertise_garage_phone} />
                      <InfoRow icon={Calendar} label="Date expertise" value={fmtDate(claim.expertise_date)} />
                    </div>
                  </div>
                )}
                ...
                {hasRepairGarage && (
                  <div className="bg-white border border-gray-200 rounded-xl p-4">
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3 flex items-center gap-1.5">
                      <Wrench size={11} className="text-purple-500" />Garage de réparation
                    </p>
                    <div className="space-y-1.5">
                      <InfoRow icon={Building2} label="Garage" value={claim.repair_garage_name} />
                      <InfoRow icon={MapPin} label="Adresse" value={claim.repair_garage_address} />
                      <InfoRow icon={Phone} label="Téléphone" value={claim.repair_garage_phone} />
                      <InfoRow icon={Calendar} label="Début" value={fmtDate(claim.repair_start_date)} />
                      <InfoRow icon={Calendar} label="Fin prévue" value={fmtDate(claim.repair_end_date)} />
                    </div>
                  </div>
                )}
                ...
                {hasIndemnisation && (
                  <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                    ...
                    <p className="text-2xl font-bold text-green-800 mb-2">{claim.indemnisation_amount?.toFixed(2)} €</p>
                    <div className="space-y-1.5">
                      <InfoRow icon={Calendar} label="Date proposition" value={fmtDate(claim.indemnisation_date)} />
                      {claim.indemnisation_paid_at && (
                        <InfoRow icon={CheckCircle} label="Paiement effectué" value={fmtDate(claim.indemnisation_paid_at)} />
                      )}
                    </div>
                  </div>
                )}
                    ...
                    {claim.events.map((ev, idx) => {
                      ...
                      return (
                        <div key={ev.id} className="relative">
                          <div className={`absolute -left-6 top-2 w-5 h-5 rounded-full flex items-center justify-center border-2 border-white
                            ${isLast ? 'bg-yellow-500' : 'bg-gray-300'}`}>
                            <EvIcon size={9} className={isLast ? 'text-white' : 'text-gray-500'} />
                          </div>
                          <div className={`rounded-lg p-3 ${isLast ? 'bg-yellow-50 border border-yellow-200' : 'bg-gray-50'}`}>
                            <div className="flex items-start justify-between gap-2">
                              <p className={`text-sm font-semibold ${isLast ? 'text-yellow-900' : 'text-gray-800'}`}>{ev.title}</p>
                              <p className="text-xs text-gray-400 flex-shrink-0 mt-0.5">{fmtDateTime(ev.event_date)}</p>
                            </div>
                            {ev.description && (
                              <p className={`text-xs mt-1 leading-relaxed ${isLast ? 'text-yellow-800' : 'text-gray-600'}`}>{ev.description}</p>
                            )}
                          </div>
                        </div>
                      );
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-gray-500 text-sm mt-0.5">Déclarez et suivez l'avancement de vos dossiers sinistres</p>
            </div>
            {!showForm && (
              <button
          ...
          {submitSuccess && (
            <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl p-4">
              ...
              <div>
                ...
                <p className="text-green-700 text-xs mt-0.5">Notre équipe prend en charge votre dossier. Vous serez informé de chaque avancement par email.</p>
              </div>
            </div>
          )}
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">Notre équipe vous contactera dans les 24h ouvrées</p>
                </div>
                <button onClick={() => { setShowForm(false); setSubmitError(null); }} className="p-1.5 hover:bg-gray-200 rounded-lg transition-colors text-gray-500">
                  <X size={16} />
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="p-5 space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1.5">Type d'incident <span className="text-red-500">*</span></label>
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1.5">Date de l'incident <span className="text-red-500">*</span></label>
                    <input type="date" required max={new Date().toISOString().split('T')[0]} value={form.incident_date}
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">Lieu de l'incident</label>
                  <input type="text" placeholder="Ex: Paris 15ème, Boulevard du Montparnasse" value={form.location}
                    onChange={e => setForm(f => ({ ...f, location: e.target.value }))}
                    className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" />
                </div>

                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">Description des faits <span className="text-red-500">*</span></label>
                  <textarea required rows={4} placeholder="Décrivez précisément les circonstances de l'incident..." value={form.description}
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <label className="flex items-center gap-2.5 cursor-pointer">
                      <input type="checkbox" checked={form.third_party_involved} onChange={e => setForm(f => ({ ...f, third_party_involved: e.target.checked }))} className="w-4 h-4 accent-yellow-500 rounded" />
                      <span className="text-sm font-medium text-gray-700">Tiers impliqué</span>
                    </label>
                    {form.third_party_involved && (
                      <input type="text" placeholder="Nom, prénom, plaque du tiers..." value={form.third_party_info}
                  ...
                  <div className="space-y-3">
                    <label className="flex items-center gap-2.5 cursor-pointer">
                      <input type="checkbox" checked={form.police_report} onChange={e => setForm(f => ({ ...f, police_report: e.target.checked }))} className="w-4 h-4 accent-yellow-500 rounded" />
                      <span className="text-sm font-medium text-gray-700">Procès-verbal établi</span>
                    </label>
                    {form.police_report && (
                      <input type="text" placeholder="Numéro du PV" value={form.police_report_number}
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-xs text-yellow-800">
                  Vous recevrez une confirmation par email. Notre équipe vous contactera dans les 24h ouvrées.
                </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-2">
                  ...
                  <button type="submit" disabled={submitting || !form.incident_type || !form.incident_date || !form.description}
              ...
              <div className="bg-gradient-to-r from-green-600 to-green-700 rounded-xl p-6 text-white">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm opacity-90">Excellente nouvelle ! Vous n'avez aucun sinistre à traiter.</p>
                  </div>
                </div>
              </div>
              ...
              {!showForm && (
                <div className="bg-white rounded-xl border border-gray-100 p-6">
                  ...
                  <div className="grid sm:grid-cols-3 gap-5">
                    {[
                      { step: '1', title: 'Déclarez en ligne', desc: 'Formulaire en 3 minutes depuis votre espace client' },
                      { step: '2', title: 'Suivi en temps réel', desc: 'Expert missionné, expertise, garage : tout est visible ici' },
                      { step: '3', title: 'Indemnisation', desc: 'Notifié dès la proposition et le versement de votre indemnité' },
                    ].map(({ step, title, desc }) => (
                      <div key={step} className="flex gap-4">
                        <div className="w-8 h-8 bg-yellow-100 rounded-lg flex items-center justify-center flex-shrink-0 font-bold text-yellow-700 text-sm">{step}</div>
                        <div>
          ...
          <div className="bg-red-50 border border-red-200 rounded-xl p-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-700 mb-3">En cas d'accident grave, contactez notre assistance disponible 24h/24, 7j/7.</p>
                <a href="tel:0180855786" className="inline-flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold text-sm transition-all">
                  <Phone size={14} />
