// Fonction pour déclencher la régénération des feeds via le webhook
export async function regenerateFeeds(): Promise<boolean> {
  try {
    ...
    const endpoint = isDev 
      ? '/api/webhook.php?action=regenFeeds'  // Use API endpoint that can be proxied
      : '/webhooks/make.php?action=regenFeeds';
