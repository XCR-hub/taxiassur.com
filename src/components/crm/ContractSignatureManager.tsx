...
interface SignatureData {
  ...
  signature_method:
    | "electronique_assureur"
    | "electronique_taxiassur"
    | "manuscrite"
    | null;
...
const SIGNATURE_METHODS = {
  electronique_assureur: {
    label: "Signature électronique assureur",
    icon: "🏢",
    color: "blue",
    description: "Via plateforme de l'assureur",
  },
  electronique_taxiassur: {
    label: "Signature électronique TaxiAssur",
    icon: "💻",
    color: "purple",
    description: "Via système TaxiAssur",
  },
  manuscrite: {
    label: "Signature manuscrite",
    icon: "✍️",
    color: "gray",
    description: "Signature papier (exception)",
  },
};

export const ContractSignatureManager: React.FC<ContractSignatureManagerProps> =
  ({ leadId, onUpdate }) => {
    ...
    const handleUploadDocument = async (
      type: "contract" | "special_conditions",
      file: File,
    ) => {
      if (
        ...
        toast.error("Seuls les contrats PDF sont acceptés");
        return;
      }
      if (file.size <= 0 || file.size > 10 * 1024 * 1024) {
        toast.error("Le document doit être un PDF non vide de 10 Mo maximum");
        return;
      }
          ...
          <div className="flex items-center gap-3">
            <div
              className={`p-3 rounded-lg ${
                signatureData?.contract_signed ? "bg-green-100" : "bg-gray-100"
              }`}
            >
              <PenTool
                className={`w-6 h-6 ${
          ...
          <div>
                ...
                : (
                  <label className="flex items-center gap-2 px-3 py-2 bg-white border-2 border-dashed border-gray-300 rounded-lg hover:border-gray-400 cursor-pointer transition-colors">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        e.target.files?.[0] &&
                        handleUploadDocument("contract", e.target.files[0])}
                    />
                  </label>
                )}
            </div>
            ...
            <div className="hidden" aria-hidden="true">
              ...
              {signatureData?.special_conditions_url
                ? (
                  <SecureDocumentLink
                    filePath={signatureData.special_conditions_url}
                    source="crm_lead_documents"
                    bucket="contract-documents"
                    fileName="Dispositions-particulieres.pdf"
                    showText
                    customText="Voir les DP"
                    className="flex items-center gap-2 px-3 py-2 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors text-sm"
                    iconSize={16}
                  />
                )
          ...
          <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <span className="text-2xl">
                      {signatureData.signature_method &&
                        SIGNATURE_METHODS[signatureData.signature_method]?.icon}
                    </span>
                    <div>
                      <span className="text-sm text-gray-900 font-medium block">
                        {signatureData.signature_method &&
                          SIGNATURE_METHODS[signatureData.signature_method]
                            ?.label}
                      </span>
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-900 font-medium">
                      {signatureData.signature_date &&
                        new Date(signatureData.signature_date)
            ...
            {signatureData.signature_verified_at && (
              <p className="text-xs text-gray-500">
                Confirmée le {new Date(signatureData.signature_verified_at)
                  .toLocaleDateString("fr-FR", {
                    day: "2-digit",
                    month: "short",
                    year: "numeric",
          ...
          <form onSubmit={handleConfirmSignature} className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
              ...
              <p className="text-xs text-blue-700">
                De nombreux assureurs proposent la signature électronique
                directement sur leur plateforme. Sélectionnez la méthode
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        setForm({
                          ...form,
                          signature_method: e.target.value as any,
                        })}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, signature_status: e.target.value })}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, signature_proof_url: e.target.value })}
                placeholder="https://..."
                className="w-full px-4 py-2 border-2 border-gray-400 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setForm({ ...form, signature_notes: e.target.value })}
