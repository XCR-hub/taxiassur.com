...
export function SubmitQuoteModal({
  ...
  const isSwisslifeRcPro = (company.name || "").toLowerCase().includes(
    "swisslife",
  );
  ...
  const [sollyOptions, setSollyOptions] = useState<SollyAzarOptions>(
    defaultSollyAzarOptions(),
  );
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      coverage_type: (existingQuote?.coverage_type as
        | ""
        | "tiers"
        | "tiers_plus"
        | "tous_risques") || "",
      ...
      includes_depannage_remorquage:
        existingQuote?.includes_depannage_remorquage ?? true,
      coverage_details: existingQuote?.coverage_details || "",
      notes: existingQuote?.notes || "",
      enrollment_fee: existingQuote?.enrollment_fee != null
        ? String(existingQuote.enrollment_fee)
        : "0",
    });
    const existingOpts = (existingQuote?.quote_options as
    ...
    setRcProAddon({
      ...
      annual: ext?.rc_pro_addon_annual != null
        ? String(ext.rc_pro_addon_annual)
        : "",
      monthly: ext?.rc_pro_addon_monthly != null
        ? String(ext.rc_pro_addon_monthly)
        : "",
    ...
    try {
      ...
      if (rcProAddon.enabled) {
        if (!rcProAddon.annual || !rcProAddon.file_url) {
          toast.warning(
            "Veuillez renseigner le prix annuel et uploader le devis RC Pro",
          );
          setSubmitting(false);
          return;
        }
        ...
        rcProMonthlyVal = !isNaN(rcMonthParsed) && rcMonthParsed > 0
          ? rcMonthParsed
          : rcProAnnualVal
          ? Math.round((rcProAnnualVal / 12) * 100) / 100
          : null;
      }
      ...
      const payload: Record<string, unknown> = {
        ...
        includes_immobilisation: isSwisslifeRcPro
          ? false
          : formData.includes_immobilisation,
        includes_assistance_0km: isSwisslifeRcPro
          ? false
          : formData.includes_assistance_0km,
        includes_rc_pro: isSwisslifeRcPro ? true : formData.includes_rc_pro,
        includes_depannage_remorquage: isSwisslifeRcPro
          ? false
          : formData.includes_depannage_remorquage,
        coverage_details: formData.coverage_details || null,
        notes: formData.notes || null,
        enrollment_fee: parseFloat(formData.enrollment_fee) || 0,
        quote_options: isSollyAzar
          ? sollyOptions
          : (existingQuote?.quote_options || {}),
      ...
      await generateNeedsAssessment({
        ...
        notes: formData.notes || null,
        quote_options: isSollyAzar
          ? (sollyOptions as unknown as Record<string, unknown>)
          : (existingQuote?.quote_options || null),
        rc_pro_addon: rcProAddon.enabled,
        rc_pro_addon_annual: rcProAnnualVal,
        rc_pro_addon_monthly: rcProMonthlyVal,
      });
    ...
    >
      <div className="lead-form space-y-5">
        <div className="bg-blue-500/10 rounded-lg p-4 border border-blue-400/40">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-blue-50 leading-relaxed">
              ...
              renseignez les conditions du devis et uploadez le PDF. La fiche de
              conseil et les documents contractuels seront automatiquement
              joints à l'email envoyé au prospect.
            </div>
          </div>
        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm text-emerald-50 leading-relaxed">
                <strong className="text-white">
                  Souscription RC Pro seule :
                </strong>{" "}
                renseignez uniquement la prime annuelle et uploadez le devis PDF
                Swisslife RC Professionnelle.
              </div>
            </div>
          </div>
        )}
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      checked={checked}
                      onChange={(e) =>
                        setFormData({ ...formData, [g.key]: e.target.checked })}
          ...
          <div className="rounded-lg p-4 border-2 border-amber-400/60 bg-amber-500/10">
            <label className="flex items-start gap-3 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  setRcProAddon({ ...rcProAddon, enabled: e.target.checked })}
              ...
              <div className="flex-1">
                ...
                <p className="text-amber-100 text-xs mt-1 leading-relaxed">
                  Complément RC Professionnelle Swisslife à joindre au devis
                  Generali (devis + contrat séparés).
                </p>
              </div>
            </label>
              ...
              <div className="mt-4 space-y-3 pl-8">
                      ...
                      onChange={(e) => {
                        ...
                        setRcProAddon({
                          ...
                          monthly: !isNaN(annualNum) && annualNum > 0
                            ? (Math.round((annualNum / 12) * 100) / 100)
                              .toString()
                            : rcProAddon.monthly,
                        });
                      }}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    <label
                      className={`flex-1 cursor-pointer rounded-lg border-2 border-dashed px-4 py-2.5 text-center transition-colors ${
                        rcProAddon.file_url
                          ? "border-emerald-500/60 bg-emerald-500/10"
                          : "border-gray-500 bg-gray-700/60 hover:border-amber-400"
                      }`}
                    >
                      <input
                        type="file"
        ...
        {isSollyAzar && (
          <div>
            ...
            <div className="bg-gray-800/70 rounded-lg p-3 border border-gray-600 space-y-1">
              {[
                {
                  key: "amenagements" as const,
                  label: "Aménagements du véhicule",
                  infoKey: "amenagements",
                },
                {
                  key: "assistance_sans_franchise" as const,
                  label:
                    "Assistance sans franchise kilométrique avec véhicule de remplacement à usage privé",
                },
                {
                  key: "bagages_marchandises" as const,
                  label: "Bagages et marchandises transportées jusqu'à 5 000 €",
                  infoKey: "bagages_marchandises",
                },
                {
                  key: "effets_personnels" as const,
                  label: "Effets et objets personnels du conducteur",
                },
                {
                  key: "equipements_pro" as const,
                  label: "Equipements professionnels",
                  infoKey: "equipements_pro",
                  hasLevel: 3 as const,
                },
                {
                  key: "indemnisation_valeur_achat" as const,
                  label:
                    "Indemnisation en valeur d'achat et/ou en valeur majorée",
                },
                {
                  key: "indemnites_journalieres" as const,
                  label:
                    "Indemnités journalières en cas d'immobilisation ou véhicule relais",
                  infoKey: "indemnites_journalieres",
                  hasLevel: 2 as const,
                },
                {
                  key: "protection_juridique" as const,
                  label: "Protection juridique",
                },
                {
                  key: "protection_conducteur_niveau2" as const,
                  label:
                    "Protection du conducteur de niveau 2 jusqu'à 500 000 €",
                },
              ].map((opt) => {
                const checked = sollyOptions[opt.key];
                const infoOpen = opt.infoKey
                  ? openInfoKey === opt.infoKey
                  : false;
                return (
                  <div key={opt.key}>
                    <label
        ...
        <div>
          ...
          />
          <p className="text-gray-300 text-xs mt-1.5">
            Valeur par défaut 0 € — modifiable par le commercial
          </p>
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            <label
              className={`flex-1 cursor-pointer rounded-lg border-2 border-dashed px-4 py-3 text-center transition-colors ${
                hasFile
                  ? "border-emerald-500/60 bg-emerald-500/10"
                  : "border-gray-500 bg-gray-700/60 hover:border-blue-400"
              }`}
            >
              <input
                type="file"
            ...
            {hasFile && (
              <button
                type="button"
                onClick={() =>
                  openPrivateDocument(formData.quote_file_url, "Devis.pdf")}
