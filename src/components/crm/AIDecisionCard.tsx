      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-3 hover:border-gray-700 transition-colors">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-gray-400">{agent.icon} {agent.name}</div>
          </div>
      ...
      <div className="p-5">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-shrink-0 flex flex-col items-center gap-0.5 relative">
            ...
            <span className="text-[9px] text-gray-400 mt-1 whitespace-nowrap">{confConfig.label}</span>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-1.5 mb-2.5">
              ...
              <span className="inline-flex items-center gap-1 text-[11px] text-gray-400 px-1">
                <Clock size={9} />
                {new Date(decision.created_at).toLocaleString('fr-FR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}
              </span>
            </div>

            <p className="text-sm text-gray-200 leading-relaxed line-clamp-2">{decision.description}</p>
            ...
            {decision.suggested_action && (
              <div className="mt-3 flex items-start gap-2 bg-gray-800/60 border border-gray-700/60 rounded-xl px-3 py-2.5">
                ...
                <p className="text-xs text-gray-200 leading-relaxed">{decision.suggested_action}</p>
              </div>
            )}
          </div>
      ...
      {(decision.rationale || (decision.data_sources && decision.data_sources.length > 0)) && (
        <>
          <button
            ...
            className="w-full flex items-center gap-2 px-5 py-2 text-[11px] text-gray-400 hover:text-gray-200 border-t border-gray-800/60 transition-colors"
          >
            ...
            <div className="px-5 pb-4 space-y-3 border-t border-gray-800/40">
              {decision.rationale && (
                <div className="bg-gray-950 border border-gray-800 rounded-xl p-4 mt-3">
                  <div className="flex items-center gap-2 mb-2.5 text-[11px] font-semibold text-gray-400 uppercase tracking-wider">
              ...
              {decision.data_sources && decision.data_sources.length > 0 && (
                <div>
                  <div className="flex items-center gap-1.5 text-[11px] text-gray-400 mb-2 font-medium uppercase tracking-wider">
                    <Database size={10} />
                    Sources utilisées
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {decision.data_sources.map((source, i) => (
                      <span key={i} className="text-[11px] bg-gray-800 border border-gray-700 text-gray-200 px-2.5 py-1 rounded-lg">
                        {source}
                      </span>
                    ))}
                  </div>
