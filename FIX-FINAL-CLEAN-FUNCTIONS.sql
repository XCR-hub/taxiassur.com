-- ============================================================================
-- FIX FINAL : Nettoyage et Recréation des Fonctions
-- ============================================================================
-- Ce script supprime toutes les anciennes versions des fonctions et les recrée proprement
...
-- Insérer/Mettre à jour la configuration
...
-- ÉTAPE 3 : Créer les fonctions (version propre)
-- ============================================================================

-- 1. Fonction get_cron_config
...
BEGIN
  RETURN QUERY
  SELECT
    bp.id, bp.title, bp.slug, bp.excerpt, bp.content, bp.published,
...
BEGIN
  RETURN QUERY
  SELECT
    n.id, n.title, n.slug, n.content, n.excerpt, n.published_at,
...
-- 4. Fonction get_faqs
CREATE FUNCTION public.get_faqs(
...
-- 6. Fonction get_dashboard_stats
...
-- 7. Fonction search_content
CREATE FUNCTION public.search_content(
  search_query text,
  content_type text DEFAULT 'all'
)
RETURNS TABLE (
  type text, id uuid, title text, slug text, excerpt text, relevance float
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  IF content_type IN ('blog', 'all') THEN
    RETURN QUERY
    SELECT
      'blog'::text, bp.id, bp.title, bp.slug, bp.excerpt,
      ts_rank(
  ...
  IF content_type IN ('news', 'all') THEN
    RETURN QUERY
    SELECT
      'news'::text, n.id, n.title, n.slug, n.excerpt,
      ts_rank(
...
BEGIN
  ...
  SELECT get_cron_config('supabase_url') INTO test_config;
  RAISE NOTICE '✓ Test 1: Configuration cron = %', LEFT(test_config, 40) || '...';

  -- Test 2: Blog posts
  SELECT COUNT(*) INTO test_count FROM get_blog_posts(10, 0);
  RAISE NOTICE '✓ Test 2: Blog posts = %', test_count;

  -- Test 3: News
  SELECT COUNT(*) INTO test_count FROM get_news(10, 0);
  RAISE NOTICE '✓ Test 3: News = %', test_count;

  -- Test 4: FAQs
  SELECT COUNT(*) INTO test_count FROM get_faqs(NULL);
  RAISE NOTICE '✓ Test 4: FAQs = %', test_count;

  -- Test 5: Leads (peut être 0 si pas de leads)
  SELECT COUNT(*) INTO test_count FROM get_leads(NULL, 10, 0);
  RAISE NOTICE '✓ Test 5: Leads = %', test_count;

  -- Test 6: Stats
  SELECT get_dashboard_stats() INTO test_stats;
  RAISE NOTICE '✓ Test 6: Dashboard stats = % articles', test_stats->>'total_blog_posts';
  ...
  RAISE NOTICE '  - /backoffice/data dans votre application';
  RAISE NOTICE '  - Les hooks React dans vos composants';
  RAISE NOTICE '  - Les fonctions RPC depuis votre frontend';
  RAISE NOTICE '';
  RAISE NOTICE 'Connexion : https://drohhxrkoequjphvabvq.supabase.co';
END $$;
