...
const dryRun = args.has("--dry-run") || process.env.DRY_RUN === "1" || process.env.INSURER_DOSSIER_DRY_RUN === "1";
...
async function main() {
  ...
  const response = await fetch(`${baseUrl()}/functions/v1/process-insurer-dossier-sends`, {
    method: "POST",
    headers: {
